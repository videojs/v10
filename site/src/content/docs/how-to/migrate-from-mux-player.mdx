...
**The player** is the outer element. It holds state, hands that state to everything inside it, and draws nothing itself. Which state it holds depends on the <DocsLink slug="concepts/features">features</DocsLink> you compose into it.

**The media** is the thing that plays the video. `<mux-video>` is the one you want: it knows what a playback ID is and how to talk to Mux. Swap it for another <DocsLink slug="concepts/media-sources">media component</DocsLink> and the rest of your player keeps working.
...
One extra step here: you build your player type up front with <DocsLink slug="reference/create-player">`createPlayer`</DocsLink>, telling it which <DocsLink slug="concepts/features">features</DocsLink> you want. A feature is a slice of player state and the behavior behind it: volume, fullscreen, captions, and so on. `videoFeatures` is the standard set for video, and it's what you want unless you're doing something unusual.
...
`createPlayer` gives you back a `Provider` to wrap your player in and a <DocsLink slug="reference/use-player">`usePlayer`</DocsLink> hook to read state from, both typed to the features you picked. Call it once, outside your component, and reuse it.
...
`content-title` reaches player state today, so your own UI can read it. No packaged skin draws it yet, so setting it alone won't put a title on screen — [Coming soon](#coming-soon) covers where that's going.
...
Be honest with yourself about this one when you plan the work. If your app leans on `onPlay`, `onTimeUpdate`, `onEnded`, and `onError`, there's no line-by-line port; those become hooks, or listeners you attach to a `ref` on the media component ([#1426](https://github.com/videojs/v10/issues/1426), [#1041](https://github.com/videojs/v10/issues/1041)).
...
The plan is for the default import to detect the content and pick the engine for you, so you won't have to think about this. Until then, the default is the safe choice.
...
The live composition is narrower than the video one on purpose: it leaves out quality, audio-track, and playback-rate state, none of which apply cleanly to a stream with no fixed duration. So a live skin's settings menu holds captions and nothing else. If you need one of the others on a live player, compose your own feature list rather than using `liveVideoFeatures`.
...
**The player will own the poster.** `poster` and its blurred placeholder are moving from the skin onto the player, and the player will resolve them through a chain: what you set, then what the media reports, then a default you supply.

That last part matters most for Mux. `<mux-video>` already derives a poster URL from the playback ID and reports it, so once the player consumes it, a playback ID gets you a poster with no URL to build — and `poster-time` becomes the declarative form of `thumbnail-time`. The storyboard track already works this way, which is why hover previews need no markup today ([#1798](https://github.com/videojs/v10/issues/1798)).

**The skins will show a title.** `content-title` already reaches player state, but nothing draws it yet. A title component and skin support are in flight ([#1123](https://github.com/videojs/v10/issues/1123)).

<FrameworkCase frameworks={["react"]}>
**`createPlayer` stops being step one.** Each preset will export a pre-composed player, so the standard case skips feature composition and `createPlayer` becomes the escape hatch for custom feature sets:

```tsx
import { Video, VideoPlayer, VideoSkin } from '@videojs/react/video';

<VideoPlayer.Provider contentTitle="Test VOD">
  <VideoSkin>{/* … */}</VideoSkin>
</VideoPlayer.Provider>
```

The preset entry point will carry the React client boundary too, so the `'use client'` directive above becomes unnecessary ([#2116](https://github.com/videojs/v10/pull/2116)).
</FrameworkCase>

**The default Mux media will pick its own engine.** Today the import path chooses, and the default is hls.js so that everything plays. Once Video.js can detect which engine a given asset needs, the default import will make that call for you and the bundle-size win becomes automatic. See [Which Mux media should you use?](#which-mux-media-should-you-use) for what to do until then.
