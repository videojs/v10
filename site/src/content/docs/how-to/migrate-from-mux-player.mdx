...
The live player adds the <DocsLink slug="reference/feature-live">live feature</DocsLink>, and the live skin adds a live button. The player leaves out playback speed, quality, and audio-track state. If you need one of those, build a player with your own feature list.
...
| Mux Player | Video.js v10 |
|---|---|
| `media.nativeEl` | a `ref` on the media component; the ref value is the `HTMLVideoElement` |
| the Video.js media object | <DocsLink slug="reference/use-media">`useMedia()`</DocsLink> inside the player |
| the hls.js instance | the `.engine` property on the media object returned by `useMedia()` |
| `prefer-playback` | `source.preferPlayback`, or pick <DocsLink slug="reference/native-hls-video">native HLS</DocsLink> outright |
</FrameworkCase>

`source.preferPlayback` is the closest mapping. Set it to `'native'` to use the browser's HLS support instead of hls.js, which is what `prefer-playback="native"` did.

Video.js has no helper for Program Date Time: no `getStartDate()` and no `currentPdt`. The player provides `liveEdgeStart` and `targetLiveWindow`. Read Program Date Time from the playback engine when you need it.
