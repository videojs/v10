...
Each piece is its own import. From the CDN, that's one script for the general-purpose <DocsLink slug="concepts/presets">preset</DocsLink> — `<video-player>` and `<video-skin>`, which already fit together — one for the Mux media, and one for analytics:
...
`VideoPlayer` is the piece that owns the state. The preset ships a matching <DocsLink slug="reference/use-player">`usePlayer`</DocsLink> hook, typed to the same state, for reading it.
