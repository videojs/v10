...
The skins use a different visual design, so you may not need to replace Mux Player's `primaryColor` and `secondaryColor` directly. Start with `--media-accent-color`. Install and restyle the skin source only when you need a closer palette match.
...
- Two skins, against Mux Player's five themes, and no runtime theme switch. That's a deliberate trade—fewer looks, each available as editable source—but it's a real difference if you shipped `theme="classic"`.
