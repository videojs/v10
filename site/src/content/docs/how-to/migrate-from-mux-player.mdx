...
Mux Player's separate primary and secondary colors don't map one-to-one to Video.js's documented properties. If your design relies on both, eject the skin and style those surfaces directly.
...
Video.js doesn't try to turn every possible UI change into another skin option. Once the packaged skin stops fitting, eject and change the components directly.
...
Two gaps to plan around. Time-slider chapter segments reflect `data-active`, but the player doesn't expose an active chapter value or `chapterchange` event, and there's no menu to jump between chapters ([#1873](https://github.com/videojs/v10/issues/1873)). If your app highlights the current chapter outside the slider, derive it from `currentTime` and `chaptersCues`. Cue points aren't implemented at all ([#1442](https://github.com/videojs/v10/issues/1442)).
...
- Chapters render on the time slider and its segments reflect `data-active`, but there's no player-level active chapter value, `chapterchange` event, or chapter menu ([#1873](https://github.com/videojs/v10/issues/1873)). No `addChapters()` either ([#1268](https://github.com/videojs/v10/issues/1268)).
