...
- **Move layout off the player provider.** Mux Player was one visible, measurable element. React's `VideoPlayer` renders no DOM of its own, and packaged HTML skins make `<video-player>` boxless with `display: contents`. Put width, height, aspect ratio, positioning, and DOM measurements on the skin's built-in <DocsLink slug="reference/player-container">container</DocsLink>, not the provider.
- **Packaged skins supply layout CSS.** Set the outer size or aspect ratio on `VideoSkin` or `<video-skin>`; the skin CSS makes its container fill that space and positions the media, controls, poster, and overlays. React needs the `skin.css` import shown above. Importing the HTML skin registers its styles.
- **Headless imports leave layout to your app.** Add `Container` or `<media-container>`, style it as the visible and positioned box, and put overlays inside it. HTML custom video hosts such as `<mux-video>` use `display: contents`, so size and measure the container rather than the media host. React media components render a native media element, but the container remains the shared layout and fullscreen target.
