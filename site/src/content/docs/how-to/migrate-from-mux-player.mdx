...
- **Chromecast is opt-in.** Follow <DocsLink slug="how-to/cast-to-airplay-and-chromecast">Cast to AirPlay and Chromecast</DocsLink> when you want it; leave the extension out when you don't.
...
Mux Player's analytics settings become attributes, properties, or props on the <DocsLink slug="reference/mux-data">Mux Data extension</DocsLink>, placed inside the player.
...
The title reaches player state, but the packaged skins don't place it in their layouts. Add the <DocsLink slug="reference/title">Title</DocsLink> component as a child of a packaged skin, or place it in a source-owned or custom layout, when you want viewers to see it:
...
`--media-accent-color` reaches the sliders, the active buttons, and the accent surfaces. Video.js derives a readable text color to sit on top of it; override that with `--media-accent-text-color` if you'd rather choose. `--media-border-radius` rounds the player, and `--media-scale-unit` scales the whole control bar at once. <DocsLink slug="how-to/customize-skins" anchor="style-a-packaged-skin">Customize skins</DocsLink> has the full list.

The skins use a different visual design, so you may not need to replace Mux Player's `primaryColor` and `secondaryColor` directly. Start with `--media-accent-color`. Restyle individual surfaces, or own the skin's source, only when you need a closer palette match.

### Level 3: own the source

For changes to controls, layout, or interactions, install an editable copy of the skin in your project. Video.js calls this **source-owned UI** because the components and styles become files you own. <DocsLink slug="how-to/customize-skins" anchor="install-and-edit-a-skin">Customize skins</DocsLink> covers the setup and available skins.
...
- Two skins, against Mux Player's five themes, and no runtime theme switch. That's a deliberate trade, fewer looks whose source you can own, but it's a real difference if you shipped `theme="classic"`.
