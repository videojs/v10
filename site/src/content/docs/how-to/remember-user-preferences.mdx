...
This assumes `isCustomElement` is already configured, as described in <DocsLink slug="how-to/use-videojs-with-vue">Use Video.js with Vue and Nuxt</DocsLink>.

```vue title="VolumePersistence.vue"
...
onMounted(() => {
  ...
  if (saved) {
    ...
    if (!restore()) {
      unsubscribeRestore = store.subscribe(() => {
        if (restore()) unsubscribeRestore();
      });
    }
  }
  ...
  // Save on change. subscribe() fires on any state change, so diff what you care about.
  let last = saved ?? {};
  unsubscribeSave = store.subscribe(() => {
    const v = selectVolume(store.state);
    if (!v || (v.volume === last.volume && v.muted === last.muted)) return;
    last = { volume: v.volume, muted: v.muted };
    save(last);
...
```svelte title="VolumePersistence.svelte"
<script lang="ts">
  import '@videojs/html/video/player';
  import type { VideoPlayerElement } from '@videojs/html/video';
  import { selectVolume } from '@videojs/html';
