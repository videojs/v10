...
#### Vue and Svelte fall back to HTML content

Vue and Svelte have no first-party Video.js package — those readers install `@videojs/html` and write custom elements. So `frameworks={["html"]}` content also renders for Vue and Svelte readers by default; you don't need to list all three.

Write a Vue- or Svelte-specific replacement only when the shared HTML content would mislead that reader — imperative wiring, lifecycle, or setup code (template refs and `onMounted`/`onUnmounted` in Vue, `bind:this` and `onMount` in Svelte). Markup-only and CSS examples stay shared, since Vue and Svelte both render the same custom elements.

When you do author a variant, exclude Vue and Svelte from the HTML block so they don't render both:
...
The same fallback applies to sidebar entries: a guide with `frameworks: ['html']` in `docs.config.ts` is already visible to Vue and Svelte readers. Add `excludeFrameworks` when a guide needs the opposite — for example, the framework-specific integration guides (<DocsLink slug="how-to/use-videojs-with-vue">Use Video.js with Vue and Nuxt</DocsLink>, <DocsLink slug="how-to/use-videojs-with-svelte">Use Video.js with Svelte and SvelteKit</DocsLink>) each set `excludeFrameworks` for the other framework so Vue readers don't see the Svelte guide in their sidebar, and vice versa.
