...
2. **How-to guides** (`src/content/docs/how-to/`): Achieve a specific outcome. These come in two shapes: **capability guides** — one page per thing the player can do (Autoplay, Play live streams, Add a quality selector), following the template below — and **task guides** — step-by-step instructions toward one specific build (self-host the player, build your own component). Address a real user goal, assume competence, and link to concept pages for deeper background. Guides assume the reader is building their own player UI with the Video.js UI library.
...
**Anything the player can do gets a capability guide.** Write a concept page when the reader needs background that applies across many guides (features, skins, security). Use this table to decide between the two:

| Concept page | How-to guide |
|---|---|
| Background shared by many guides | One capability or one build |
...
Keep each page's job clear. A capability guide leads with working code and includes only the explanation a reader needs to modify it — that's its "How it works" section. Deeper background belongs in a concept page; link to it instead of explaining inline. Step-by-step instructions creeping into a concept page belong in a how-to guide.

### Capability guide template

Give a capability its own guide when developers search for it by name, browser or platform rules affect it, and it takes several APIs explained together. Otherwise it stays a section of a broader guide (volume lives in Playback; chapters live in Text Tracks).

Every capability guide uses the same sections, in this order:
...
A guide may skip a section that doesn't apply, but the sections it has keep these names and this order. One capability per page. Titles are goal-oriented and in the reader's words, not ours: each completes the sentence "How to…" without the literal prefix ("Play live streams", "Show captions and subtitles" — not "Live Playback" or "Text Tracks"). Think about what someone who only knows YouTube — or their coding agent — would search for. One page should fully answer one question.
...
Capability guides also include a few extra frontmatter fields that help search engines and coding agents find the page:
...
- **`category`** — `'capability'` for capability guides
- **`components`** — the UI components the guide composes
- **`api`** — the features and APIs the guide uses
- **`keywords`** — search phrases a developer or agent would use to find this capability
