...
2. **How-to guides** (`src/content/docs/how-to/`): Achieve a specific outcome — one page per thing the reader wants to do (Autoplay, Play live streams, Self-host the player), following the template below. Address a real user goal, assume competence, and link to concept pages for deeper background. Guides assume the reader is building their own player UI with the Video.js UI library.
...
**Anything the player can do gets a how-to guide.** Write a concept page when the reader needs background that applies across many guides (features, skins, security). Use this table to decide between the two:
...
Every how-to guide uses the same sections, in this order:
...
A guide may skip a section that doesn't apply, but the sections it has keep these names and this order. One goal per page. Titles are goal-oriented and in the reader's words, not ours: each completes the sentence "How to…" without the literal prefix ("Play live streams", "Show captions and subtitles" — not "Live Playback" or "Text Tracks"). Think about what someone who only knows YouTube — or their coding agent — would search for. One page should fully answer one question.
