...
The skin above is one import from the package. To edit its controls, markup, or styles, install the skin's source from the Shadcn registry instead. Add the `@videojs` namespace to your `components.json`, then add the skin your choices above selected:
