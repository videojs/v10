...
The CDN tab loads the player from a public CDN. To serve everything from your own origin instead — for offline, air-gapped, or locked-down network deployments — pick one of the options below.
...
Load the output as a module — the same `type="module"` requirement as the CDN path applies:

```html
<script type="module" src="/player.js"></script>
```

You get one file (plus a sourcemap), no runtime requests to a CDN, and only the features you import. Any bundler works — Vite, Rollup, or webpack — as long as the output stays ESM.
...
The CDN entry files (`video.js`, `media/hls-video.js`, …) are not standalone. They import shared, content-hashed chunks (`default-*.js`, `ui-*.js`, …) that live alongside them. Copying a single file breaks at runtime — mirror the entire `cdn/` directory and keep its layout intact.
</Aside>
...
Then point each script tag at your copy instead of the CDN:
...
`@videojs/react` is bundled into your app like any other dependency, so your normal production build already serves the player from your own origin with no CDN or runtime fetches. No extra steps are needed for offline or self-hosted deployments.
