...
The `--framework` flag takes `html` or `react`. Pass `--framework html`: it generates the `@videojs/html` custom elements, which are the ones this framework uses. Move the generated imports and markup into a component, as shown below.
</FrameworkCase>
</div>
