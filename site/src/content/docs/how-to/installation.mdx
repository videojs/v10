...
The `--framework` flag takes `html` or `react`. For a Vue or Svelte project, pass `--framework html`: it generates the `@videojs/html` custom elements those projects use. Move the generated imports and markup into a component, as shown below.
</FrameworkCase>
</div>
...
Video.js aims to provide idiomatic development experiences: React components, or HTML custom elements you write in a plain page, a Vue template, or a Svelte component.
...
<FrameworkCase frameworks={["vue"]}>
  In Vue, you'll install the `@videojs/html` custom elements and write them in your templates. They're first-class there: Vue binds their attributes and listens to their events like any other element. Complete the installation steps below, then read the Vue guide for compiler configuration, player state, and Nuxt.

  <DocsLinkCard slug="how-to/use-videojs-with-vue">Vue and Nuxt</DocsLinkCard>
</FrameworkCase>

<FrameworkCase frameworks={["svelte"]}>
  In Svelte, you'll install the `@videojs/html` custom elements and write them in your markup. They're first-class there: Svelte renders them as real elements and assigns object values to their properties. Complete the installation steps below, then read the Svelte guide for player state, styling, and SvelteKit.
...
Import the elements in a single-file component, take the source as a prop, and bind it on the media element. Vue needs one build change to leave these tags alone, which the `vite.config.ts` tab covers.
...
Without `isCustomElement`, Vue resolves hyphenated tags as components and warns that it can't find them. The `vite.config.ts` tab lists exactly the tags this player renders — add a tag there whenever you add one to a template. <DocsLink slug="how-to/use-videojs-with-vue">Use Video.js with Vue and Nuxt</DocsLink> has the Nuxt equivalent, plus how to read player state and pass objects as properties.
</Aside>
</FrameworkCase>
...
Import the elements in a component, take the source as a prop, and bind it on the media element. Svelte compiles these tags to real custom elements, with no compiler configuration to add.
