...
|---|---|
| `controls` | The <DocsLink slug="concepts/skins">skins</DocsLink> include the common controls, laid out in a familiar way that users would expect. Not every Plyr control ships in every skin—the video skins do not include `rewind` and `fast-forward` buttons, for example. To change which controls appear, or to customize the skin beyond basic colors, [install the skin source](#install-the-skin-source) and change controls, layout, styles, or icons. |
| `rewind`, `fast-forward`, `seekTime` | The audio skins include 10-second skip buttons; the video skins do not. [Install the skin source](#install-the-skin-source) and add a <DocsLink slug="reference/seek-button">SeekButton</DocsLink>, which skips by its `seconds` value (default `30`; negative values seek backward). |
...
| `clickToPlay` | Preset video skins include click and tap gestures. [Install the skin source](#install-the-skin-source) to remove or change them. |
| `keyboard` | Preset video skins include common hotkeys. [Install the skin source](#install-the-skin-source) to remove or change them. |
| `tooltips` | Preset skins include tooltips for common controls. [Install the skin source](#install-the-skin-source) to customize them. |
...
Plyr's `controls` option chooses which controls appear. Video.js skins come with their own control set and layout. Keep the skin when that UI fits your player. To remove, reorder, or restyle its controls, [install the skin source](#install-the-skin-source) and edit the owned implementation. Adding a child to a skin does not place it inside the control bar.
...
Video.js v10 skins offer similar color customization via CSS custom properties. [Install the skin source](#install-the-skin-source) when you need deeper control over layout, control structure, icons, or interaction styling.
