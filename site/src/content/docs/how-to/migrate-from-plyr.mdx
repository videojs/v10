...
If you need extra customization of layout, styles, or icons, make the skin Source-Owned UI: install its source into your application. It is a Shadcn installation: register the `@videojs` namespace with `npx shadcn@latest registry add`, run `npx shadcn@latest add @videojs/video`, then modify the installed components and styles directly. <DocsLink slug="how-to/install-skins-from-the-registry">Install skins from the Shadcn registry</DocsLink> walks through it for every packaged skin.
