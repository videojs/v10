...
- **The UI is composable.** Buttons, sliders, menus, gestures, and hotkeys are individual accessible components. Start with the Minimal or Default skin, then [own its source](#source-owned-ui) when you need to change the structure.
- **Remote playback is built in.** The video skins include AirPlay and Cast controls. Follow <DocsLink slug="how-to/cast-to-airplay-and-chromecast">Cast to AirPlay and Chromecast</DocsLink> to add the Google Cast extension.
...
|---|---|
| `controls` | The <DocsLink slug="concepts/skins">skins</DocsLink> include the common controls, laid out in a familiar way that users would expect. Not every Plyr control ships in every skin—the video skins do not include `rewind` and `fast-forward` buttons, for example. To change which controls appear, or to customize the skin beyond basic colors, [own the skin's source](#source-owned-ui) and change controls, layout, styles, or icons. |
| `rewind`, `fast-forward`, `seekTime` | The audio skins include 10-second skip buttons; the video skins do not. [Own the skin's source](#source-owned-ui) and add a <DocsLink slug="reference/seek-button">SeekButton</DocsLink>, which skips by its `seconds` value (default `30`; negative values seek backward). |
...
| `clickToPlay` | Preset video skins include click and tap gestures. [Own the skin's source](#source-owned-ui) to remove or change them. |
| `keyboard` | Preset video skins include common hotkeys. [Own the skin's source](#source-owned-ui) to remove or change them. |
| `tooltips` | Preset skins include tooltips for common controls. [Own the skin's source](#source-owned-ui) to customize them. |
...
<FrameworkCase frameworks={["html"]}>
Individual HTML buttons include no text, icon, or finished styling. Add your own content, or import `@videojs/html/icons/element` and place a `<media-icon>` inside the button. Attach click handlers directly to controls you add, and keep playback event handlers on the `<video>` or `<audio>`. To change or listen to a button inside a ready-made skin, own the skin's source first.
</FrameworkCase>

<FrameworkCase frameworks={["react"]}>
Individual React buttons do not include visible content. Use their `render` props to add an icon or text and style the element you return. Install a ready-made skin's source when you need to change its control set or layout.
</FrameworkCase>
...
Video.js v10 skins offer similar color customization via CSS custom properties. [Own the skin's source](#source-owned-ui) when you need deeper control over layout, control structure, icons, or interaction styling.
