...
Replace `<video>` with `<vimeo-video>`. If you're using the CDN, add an additional script:
...
`--media-accent-color` reaches the sliders, active buttons, and accent surfaces. To recolor icons and text instead, use `--media-color-primary`. See <DocsLink slug="how-to/customize-skins">Customize skins</DocsLink> for the full list.
