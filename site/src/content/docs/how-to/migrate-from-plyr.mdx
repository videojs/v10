...
- `@videojs/react/video` is a <DocsLink slug="concepts/presets">preset</DocsLink>: a player, a skin, and a media element already composed from the standard set of <DocsLink slug="concepts/features">features</DocsLink> for video. `VideoPlayer` is the piece that owns the state.
