...
Then create a reusable `VideoPlayer` component in your app:
...
- `createPlayer` builds a typed player from a set of <DocsLink slug="concepts/features">features</DocsLink>. `videoFeatures` is the standard composition for video. Call it once, outside your component, and reuse it.
...
<Aside type="note">
Vimeo and YouTube don't have prebuilt CDN scripts yet, so these two need a bundler.
</Aside>
...
<FrameworkCase frameworks={["react"]}>
Use the <DocsLink slug="reference/use-player">`usePlayer`</DocsLink> hook that <DocsLink slug="reference/create-player">`createPlayer`</DocsLink> returns, or the feature selectors:
...
**The player will own the poster.** `poster` and its blurred placeholder are moving from the skin onto the player, so a media component can report a poster and the player can resolve it. Plyr's `data-poster` maps more directly once that lands ([#1798](https://github.com/videojs/v10/issues/1798)).

**The skins will show a title.** `content-title` already reaches player state, but nothing draws it yet ([#1123](https://github.com/videojs/v10/issues/1123)).

**A Plyr skin is on the list.** Minimal is the closest starting point today, but a skin that matches Plyr's look is tracked at [#181](https://github.com/videojs/v10/issues/181).
