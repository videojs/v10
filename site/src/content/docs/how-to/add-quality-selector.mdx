...
category: 'capability'
components: ['QualityRadioGroup', 'Menu']
api: ['qualityFeature', 'useQualityOptions']
keywords: ['video quality selector', 'renditions', 'adaptive bitrate', 'abr', 'resolution menu']
---
...
Quality selection applies to streaming sources. A progressive MP4 has one rendition; HLS and DASH streams offer several. See <DocsLink slug="how-to/load-different-file-formats">Load different file formats</DocsLink>.
...
- Quality availability is `'unavailable'` for media that doesn't expose renditions — progressive files, and streams before the manifest loads. Render quality UI conditionally.
- Renditions come from the streaming engine, so the list depends on what the manifest declares. A single-rendition stream offers no meaningful selection.
...
### The menu shows one entry plus Auto

The manifest declares a single rendition. Encode the stream as a multi-rendition ladder to give the engine and users something to choose between.
