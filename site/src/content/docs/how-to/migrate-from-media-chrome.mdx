...
Media Chrome's React package wraps its custom elements. `@videojs/react` ships native components composed with <DocsLink slug="reference/create-player">`createPlayer`</DocsLink> instead.
...
`createPlayer` takes the <DocsLink slug="concepts/features">features</DocsLink> your player needs and returns a typed `Player` component and `usePlayer` hook. Call it once, outside your component, and reuse it. `Container` is the box the media and controls live in, and it's shared rather than per-player, so it comes from the package root.

If you don't need a custom feature list, a <DocsLink slug="concepts/presets">preset</DocsLink> hands you the same pair already configured: `VideoPlayer` and `usePlayer` from `@videojs/react/video`.
