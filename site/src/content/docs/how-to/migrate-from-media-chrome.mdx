...
| `lang` | i18n translator | Localization is configured through the <DocsLink slug="how-to/internationalize-the-player">translator and locale registry</DocsLink>. |
...
| `--media-secondary-color` | no direct equivalent | Author CSS or own the skin's source when you need this palette role. |
| `--media-control-background` | no direct equivalent | Author CSS or own the skin's source. |
| `--media-control-hover-background` | no direct equivalent | Author CSS or own the skin's source. |
| `--media-range-track-height` | no direct equivalent | Author slider CSS or own the skin's source. |
...
Keep the skin when its CSS variables and content slots cover your changes. Build a custom control bar when you want to start from scratch. Own the skin's source when you want its existing layout and styles but need to change the controls inside it.
</FrameworkCase>
...
- <DocsLink slug="concepts/skins">Skins</DocsLink> and <DocsLink slug="how-to/change-a-skins-controls-and-layout">Change a skin's controls and layout</DocsLink>
