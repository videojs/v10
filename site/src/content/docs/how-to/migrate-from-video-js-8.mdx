...
Keep the generated player and UI registration imports. The installed HTML block registers the elements its source-owned layout uses; you do not render the packaged skin element itself:
...
Owning the source also means owning its styles. The registry installs Tailwind or generated vanilla CSS beside the markup; a bare layout renders unstyled until you bring those styles along.

## Plugins

v10 has no plugin system. There's no `videojs.registerPlugin`, no `player.myPlugin()`, and no plugin lifecycle.

That's a deliberate choice rather than a missing feature. v8 plugins existed largely because there was no other way in: to add a control, change a behavior, or support a format, you had to reach into player internals through the one door the plugin API provided. v10 gives you the front door instead: components you can add, skin source you can own, and media elements you can swap. Most of what plugins did is now ordinary composition.
...
- **UI additions** (extra buttons, overlays, custom control bars). Rebuild them in a source-owned skin, or with <DocsLink slug="how-to/build-your-own-component">your own component</DocsLink>. This is usually less code than the plugin was.
