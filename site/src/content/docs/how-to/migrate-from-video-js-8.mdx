...
- **The poster is on the player, not the media.** It's how v10 handles anything a component other than the media has to read.
...
- **The poster is on the provider, not the media.** It's how v10 handles anything a component other than the media has to read.
...
Posters and titles live on the player because more than one component wants them. The skin paints the poster, and the title region reads the title, so neither has to reach into your media element to find out. Each resolves through a chain: what you set, then what the media reports about itself, then a default you supply. That's how a media component like the Mux one can hand you a poster you never wrote down.

You can also set a poster placeholder on the player: a blurred stand-in shown while the real poster loads, as a CSS image with the blur baked in. v8 had no equivalent. See the <DocsLink slug="reference/poster">Poster</DocsLink> reference.
