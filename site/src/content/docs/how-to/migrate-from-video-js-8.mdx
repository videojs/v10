...
v10 ships web components, so the migration keeps its declarative feel. One script from the CDN gives you `<video-player>` and `<video-skin>`, the general-purpose <DocsLink slug="concepts/presets">preset</DocsLink>: a player, a skin, and a media element that already fit together.
...
`VideoPlayer` is the piece that owns the state. The preset ships a matching <DocsLink slug="reference/use-player">`usePlayer`</DocsLink> hook, typed to the same state, for reading it. There's no ref to manage and no disposal to remember.
...
Drop `VideoSkin` and compose the <DocsLink slug="concepts/ui-components">UI components</DocsLink> yourself. `Container` is the element the skin used to render for you: it's the box the media and controls live in. Compound parts are namespaced, so a time slider is assembled from `TimeSlider.Root`, `TimeSlider.Track`, and friends:
...
There are **no `on*` props**. Read state through the preset's <DocsLink slug="reference/use-player">`usePlayer`</DocsLink> hook, which is typed to the features that preset includes:
...
<FrameworkCase frameworks={["html"]}>
That's what <DocsLink slug="reference/html-create-player">`createPlayer`</DocsLink> is for: hand it the features you want and it gives you back the mixins to define your own player element.
</FrameworkCase>

<FrameworkCase frameworks={["react"]}>
That's what <DocsLink slug="reference/create-player">`createPlayer`</DocsLink> is for: hand it the features you want and it gives you back a `Player` component and a `usePlayer` hook typed to them, the same pair every preset is built from.
</FrameworkCase>
