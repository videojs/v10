...
Drop `VideoSkin` and compose the <DocsLink slug="concepts/ui-components">UI components</DocsLink> inside your provider yourself. Compound parts are namespaced, so a time slider is assembled from `TimeSlider.Root`, `TimeSlider.Track`, and friends:

```tsx
import { Controls, createPlayer, Hotkey, MuteButton, PlayButton, TimeSlider } from '@videojs/react';
...
The live composition is deliberately narrower than the video one. It leaves out playback rate, quality, and audio-track state, so those controls don't appear in a live skin's settings menu. If you need one of them on a live player, compose your own feature list instead of using the preset.

## Coming soon

Changes in flight that touch this guide. None has shipped, so everything above is what works today.
...
<FrameworkCase frameworks={["react"]}>
**`createPlayer` stops being step one.** Each preset will export a pre-composed player, so the standard case skips feature composition entirely and `createPlayer` becomes the escape hatch for custom feature sets:

```tsx
import { Video, VideoPlayer, VideoSkin } from '@videojs/react/video';

<VideoPlayer.Provider contentTitle="Movie title">
  <VideoSkin>
    <Video src="/video.mp4" playsInline />
  </VideoSkin>
...
The preset entry point will carry the React client boundary too, so the `'use client'` directive in the examples above becomes unnecessary ([#2116](https://github.com/videojs/v10/pull/2116)).
</FrameworkCase>
...
- React has no `on*` event props. Use `usePlayer` and selectors, or listeners on a media ref.
