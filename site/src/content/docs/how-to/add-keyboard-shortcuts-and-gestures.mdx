...
category: 'capability'
components: ['Hotkey', 'Gesture', 'StatusAnnouncer']
api: ['playbackFeature', 'volumeFeature', 'fullscreenFeature']
keywords: ['keyboard shortcuts video', 'hotkeys', 'double tap seek', 'video gestures', 'spacebar play pause']
---
...
```tsx
import { createPlayer, Gesture, Hotkey } from '@videojs/react';
...
- `Gesture` binds pointer input — `tap` and `doubletap` — to the same actions, scoped by `pointer` (`mouse` or `touch`) and `region` (`left`, `center`, `right`), so double-tap-left rewinds while double-tap-right skips forward.
