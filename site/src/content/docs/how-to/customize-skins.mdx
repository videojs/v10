...
<FrameworkCase frameworks={["react"]}>
The skin's files are under `components/videojs/skins/<preset>/<theme>`. Keep importing the player and media from `@videojs/react`. Import the installed skin component from `skin.tsx`, then render it around your media where you previously rendered the packaged skin.
</FrameworkCase>

<FrameworkCase frameworks={["html"]}>
The skin's files are under `components/videojs/skins/<preset>/<theme>`. Keep importing the player and media from `@videojs/html`. Import the installed `skin.ts` once, then paste the markup from `skin.html` inside your player around the media element.
</FrameworkCase>
...
| Change only the Minimal theme or one media preset | `components/videojs/styles/themes/minimal.css`, `video.css`, or `audio.css` |
