...
`useSlider` is the low-level hook behind the <DocsLink slug="components/slider">Slider</DocsLink>, <DocsLink slug="components/time-slider">TimeSlider</DocsLink>, and <DocsLink slug="components/volume-slider">VolumeSlider</DocsLink> components. Use it when you need to build a new slider primitive with custom state calculations. For standard player controls, use the built-in components.
