...
Add `streamTypeFeature` to a custom player when you need `streamType` in player state. The live presets separately include the <DocsLink slug="api/feature-live">live feature</DocsLink>, which tracks the live edge.
...
Pass `selectStreamType` to <DocsLink slug="api/use-player">`usePlayer`</DocsLink> to subscribe to stream type state. Returns `undefined` if the stream type feature is not configured.
</FrameworkCase>

<FrameworkCase frameworks={["html"]}>
Pass `selectStreamType` to <DocsLink slug="api/player-controller">`PlayerController`</DocsLink> to subscribe to stream type state. Returns `undefined` if the stream type feature is not configured.
</FrameworkCase>
