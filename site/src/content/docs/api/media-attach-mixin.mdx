...
`MediaAttachMixin` creates a class that registers itself with the <DocsLink slug="components/player">player</DocsLink> on connect. Apply it to custom media elements so they automatically wire to the store.
...
`MediaAttachMixin` is pre-configured with the default `mediaContext` published by <DocsLink slug="components/player">`PlayerElement`</DocsLink>. Call `createMediaAttachMixin` only when a custom player publishes a matching custom `MediaContext`:
