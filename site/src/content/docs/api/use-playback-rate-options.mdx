...
`usePlaybackRateOptions` returns the current playback rate, the available options, and `setRate` / `setValue` callbacks for wiring rate selection into a menu. It returns `null` when the <DocsLink slug="api/feature-playback-rate">playback rate feature</DocsLink> is not configured.
