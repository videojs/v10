...
The built-in selectors (<DocsLink slug="api/feature-playback">`selectPlayback`</DocsLink>, <DocsLink slug="api/feature-buffer">`selectBuffer`</DocsLink>, etc.) are all created with `createSelector`. Use it to create selectors for custom slices.
...
Pass selectors to <DocsLink slug="api/use-player">`usePlayer`</DocsLink> or <DocsLink slug="api/use-store">`useStore`</DocsLink> for reactive subscriptions.
</FrameworkCase>

<FrameworkCase frameworks={["html"]}>
Pass selectors to <DocsLink slug="api/player-controller">`PlayerController`</DocsLink> or <DocsLink slug="api/store-controller">`StoreController`</DocsLink> for reactive subscriptions.
</FrameworkCase>
