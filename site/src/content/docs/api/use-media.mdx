...
With a packaged preset, import `useMedia` from `@videojs/react`. With a player made by <DocsLink slug="api/create-player">`createPlayer`</DocsLink>, use the `useMedia` hook returned by that call. It is connected to the new player's context.
...
To attach your own media object, see <DocsLink slug="api/use-media-attach">`useMediaAttach`</DocsLink>.
