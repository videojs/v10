...
`useCaptionsOptions` returns the currently showing caption track, the available options (starting with an `Off` option), and a `setValue` callback for wiring caption selection into a menu. It returns `null` when the <DocsLink slug="api/feature-text-tracks">text tracks feature</DocsLink> is not configured.
