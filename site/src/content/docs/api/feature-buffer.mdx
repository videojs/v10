...
During live playback, `seekable` tells you which times remain available. The first start is the oldest available time, and the last end is the newest. Both values can move forward as a sliding live stream drops older video and adds new video. See <DocsLink slug="api/feature-live">live state</DocsLink> for the live-edge values.
...
Pass `selectBuffer` to <DocsLink slug="api/use-player">`usePlayer`</DocsLink> to subscribe to buffer state. Returns `undefined` if the buffer feature is not configured.
</FrameworkCase>

<FrameworkCase frameworks={["html"]}>
Pass `selectBuffer` to <DocsLink slug="api/player-controller">`PlayerController`</DocsLink> to subscribe to buffer state. Returns `undefined` if the buffer feature is not configured.
</FrameworkCase>
