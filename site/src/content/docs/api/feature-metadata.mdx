...
Pass `selectMetadata` to <DocsLink slug="api/use-player">`usePlayer`</DocsLink> to subscribe to metadata state. Returns `undefined` if the metadata feature is not configured.
...
Pass `selectMetadata` to <DocsLink slug="api/player-controller">`PlayerController`</DocsLink> to subscribe to metadata state. Returns `undefined` if the metadata feature is not configured.
