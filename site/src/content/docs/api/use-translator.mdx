...
`useTranslator` returns the translator from the nearest <DocsLink slug="api/i18n-provider">`I18nProvider`</DocsLink>. Control components pass text descriptors from core `getLabel()`. Custom UI can pass an opaque key with its English fallback.
