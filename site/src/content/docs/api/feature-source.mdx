...
Pass `selectSource` to <DocsLink slug="api/use-player">`usePlayer`</DocsLink> to subscribe to source state. Returns `undefined` if the source feature is not configured.
</FrameworkCase>

<FrameworkCase frameworks={["html"]}>
Pass `selectSource` to <DocsLink slug="api/player-controller">`PlayerController`</DocsLink> to subscribe to source state. Returns `undefined` if the source feature is not configured.
</FrameworkCase>
