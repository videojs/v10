...
A unit test, `pnpm -F site test diataxis`, guards the line between the two folders: no title starts with "How to", no reference page carries how-to sections or a `<CustomUiNote />`, every page is in the sidebar, and no redirected slug still exists. Whether a guide reads as a concept or a how-to is a judgement you make while writing and reviewing, with the compass below. The test helps by printing, as warnings only, any heading on a Concepts-section page that opens with a task verb such as "Add" or "Configure".
...
When a page moves to a different folder, keep its old address alive by listing the old slug on the new entry. The site turns each one into a redirect for every framework the page renders in, for both the page and its `.md` twin:
