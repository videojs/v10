...
This guide covers how to create API reference pages — component references, media references, and util references (hooks, controllers, mixins) under `reference/` in the docs content.
...
Add `stability: experimental` when the individual API is available for testing but may still change. The whole of v10 is in beta, so "beta" on one page says nothing; "experimental" marks the APIs that are less settled than the rest. The page header and sidebar display the status automatically.
...
The component renders only those package imports. UI components have no per-component CDN bundle, and that story belongs to the <DocsLink slug="reference/cdn">CDN</DocsLink> reference, so individual pages never make CDN claims of their own.
...
Unlike UI components, every media module ships a matching CDN bundle, so for HTML readers the component names that bundle and links the <DocsLink slug="reference/cdn">CDN</DocsLink> reference. A unit test enforces the Import section on every page that renders `<MediaReference />`.

## Create an extension reference page

Extension reference pages document the <DocsLink slug="concepts/overview">extensions</DocsLink> that add optional behavior to a player's media, such as Google Cast and Mux Data. They are hand-authored today: no generator produces their API tables.
