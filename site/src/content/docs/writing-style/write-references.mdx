...
This guide covers component references, media references, and util references (hooks, controllers, mixins) under `reference/` in the docs content. Legacy error-code pages are also under `reference/api/`; their content comes from `video.js/errors` rather than the API docs builder.
...
| HTML element | `element.ts` | `packages/html/src/ui/play-button/element.ts` |
...
Every file in a component directory uses a simple role name; helpers drop the folder prefix, such as
`slider/segments.ts`. Additional part-scoped data attribute files also use a simple name, such as `menu/item.ts`, and
are discovered by the `@parts` JSDoc tag on an exported const whose name ends in `DataAttrs`.
...
| --- | --- |
| Default M4A | `{{VJS10_DEMO_AUDIO_M4A}}` |
...
Create the page under `site/src/content/docs/reference/components/` for a UI, skin, media, or extension component, or under `site/src/content/docs/reference/api/` for a hook, feature, controller, factory, or utility. The file is `{name}.mdx` with this structure:
...
Add `stability: unstable` when an API works but its shape may still change in a minor release. The page header, the sidebar, and an explanatory note at the top of the page show the status automatically.
...
1. **Import** — render the public React export and the HTML registration module with `<ComponentImports>`.
...
The component renders only those package imports. UI components have no per-component CDN bundle, and that story belongs to the <DocsLink slug="guides/cdn">CDN</DocsLink> guide, so individual pages never make CDN claims of their own.
...
Unlike UI components, every media module ships a matching CDN bundle, so for HTML readers the component names that bundle and links the <DocsLink slug="guides/cdn">CDN</DocsLink> guide. A unit test enforces the Import section on every page that renders `<MediaReference />`.

## Create an extension reference page

Extension reference pages document the <DocsLink slug="guides/architecture">extensions</DocsLink> that add optional behavior to a player's media, such as Google Cast and Mux Data. They are hand-authored today: no generator produces their API tables.

Every extension page opens with an `## Import` section rendered by `<ExtensionImports>`, placed after the intro prose and before the first body section:

```mdx
## Import

<ExtensionImports extension="mux-data" react="MuxData" />
```

`extension` is the module name: the suffix of `@videojs/react/extensions/{extension}` and `@videojs/html/extensions/{extension}`, and of the CDN bundle `extensions/{extension}.js`. `react` is the named export from the React subpath. A unit test enforces the section on every extension reference page.
...
- **Extensions** — extensions that add optional behavior to the player's media, such as Google Cast and Mux Data
...
Skins, UI Components, Media Components, Extensions, and Player Features are flat by design. Do not add nested subsections to them. Mark framework-specific entries with `frameworks: ['react']` or `frameworks: ['html']` on the entry itself.
...
```ts
{
  ...
  contents: [
    ...
    { slug: 'reference/components/your-component' }, // add here
  ],
},
```
...
**Component pages:**
- <DocsLink slug="reference/components/play-button">PlayButton</DocsLink> — single-part, interactive
- <DocsLink slug="reference/components/controls">Controls</DocsLink> — behavior-heavy (auto-hide)
- <DocsLink slug="reference/components/time">Time</DocsLink> — multi-part, formatting

**Util pages:**
- <DocsLink slug="reference/api/use-player">usePlayer</DocsLink> — React hook, multi-overload
- <DocsLink slug="reference/api/player-controller">PlayerController</DocsLink> — HTML controller
