...
description: Style a packaged Video.js skin or add its source to change controls, layout, styles, and interactions
---
...
```html
<video-skin style="--media-accent-color: #ff3e00; --media-border-radius: 1rem">
...
| ------------- | --------------- | ----- | ------- |
| `--media-accent-color` | Slider fills, highlighted controls and menu items, and primary actions | [`<color>`](https://developer.mozilla.org/en-US/docs/Web/CSS/color_value) | `#ff3e00` |
...
### Verify the accent in a browser

The accent is easiest to see in a filled time or volume slider and on a highlighted control or menu item. A video paused at the start may show no time-slider fill, and idle buttons need not show the accent.

1. Load a playable video and seek away from the start. Check that the filled part of the time slider uses `#ff3e00`.
2. Move the pointer over a control, such as Play or Settings, or focus it with the keyboard. If the settings menu is available, open it and highlight an item. Check the highlighted background and its contrasting text or icon.
3. Compare the same playback position and interaction state before and after your change. If you inspect styles in browser developer tools, check the rendered fill or highlight, including its `::before` or `::after` layer when present. Reading the custom property's value alone does not confirm a visible change.

`--media-accent-color` does not set the control bar background. To change that surface, [add the skin source](#style-skin-source) and edit its styles.
...
<FrameworkCase frameworks={["html"]}>
See <DocsLink slug="reference/components/poster">`<media-poster>`</DocsLink>.
</FrameworkCase>
...
<DocsLinkCard slug="guides/installation-shadcn" description="Add editable skin source and build a working React or HTML player">Install Video.js with Shadcn</DocsLinkCard>
...
| Goal | Where to start |
| ---- | -------------- |
| Add, remove, or rearrange controls | `skin.tsx` |
