...
<span id="eject-and-edit-a-skin" aria-hidden="true"></span>
<span id="set-up-the-shadcn-cli" aria-hidden="true"></span>
...
Video.js publishes the skin files in an installable catalog. The [`shadcn` CLI](https://ui.shadcn.com/docs/cli) copies those files into your project, whether you use React or HTML.
...
The Shadcn installation guide lets you choose React or HTML, the skin, styling approach, theme, and media source. It then adds the skin source and shows how to use it in a working player.
...
<FrameworkCase frameworks={["react"]}>
The files are under `components/videojs/<preset>`. Keep importing the player and media from `@videojs/react`. Import the installed skin component from `skin.tsx`, then render it around your media where you previously rendered the packaged skin.
</FrameworkCase>

<FrameworkCase frameworks={["html"]}>
The files are under `components/videojs/<preset>`. Keep importing the player and media from `@videojs/html`. Import the installed `skin.ts` once, then paste the markup from `skin.html` inside your player around the media element.
</FrameworkCase>
...
### Inspect or update the skin files
