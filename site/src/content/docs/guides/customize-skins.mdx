...
| ------------- | --------------- | ----- | ------- |
| `--media-accent-color` | Time and volume slider fills; controls on hover, keyboard focus, or while expanded; highlighted menu items; primary dialog buttons. Most idle controls and the control bar background do not change. [Edit skin source](#style-skin-source) to restyle the bar. | [`<color>`](https://developer.mozilla.org/en-US/docs/Web/CSS/color_value) | `#f5c518` |
...
1. Load a playable video and seek past the start. Check that the filled part of the time slider uses your accent color; at 0:00, there may be no fill to see.
2. Hover over a control, focus it with the keyboard, or highlight an available settings-menu item. Compare the same playback position and interaction state before and after your change.
3. If you inspect styles in browser developer tools, check the rendered background, including its `::before` layer when present. Reading the custom property's value alone does not confirm a visible change.
