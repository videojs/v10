...
| Add, remove, rearrange, or deeply restyle controls; change layout or interactions | [Eject and edit a skin](#eject-and-edit-a-skin). Ejecting copies the skin's source files into your project so you can change them. |
...
The media element and poster share `--media-object-fit` and `--media-object-position`, so a `cover` poster stays aligned with `cover` video. See <DocsLink slug="guides/poster">Add a poster and loading placeholder</DocsLink>.
...
See <DocsLink slug="reference/components/poster">Poster</DocsLink>.

Packaged skins do not provide settings for adding, removing, or rearranging controls, or for replacing built-in feedback and interactions. To make those changes, [eject the closest skin](#eject-and-edit-a-skin) and edit its source.
...
### Choose a theme and connect the CLI to Video.js

Choose one theme for the Video.js source in your project. The command saves `@videojs` as a short prefix in `components.json`, so later commands can use names such as `@videojs/video`.

For the Default theme:

<RegistryCommand />

For the Minimal theme:

<RegistryCommand theme="minimal" />

### Install a skin

Install the preset closest to the interface you want. Item names stay the same in both theme catalogs:
...
<FrameworkCase frameworks={["react"]}>
The skin's files are under `components/videojs/<preset>`. Keep importing the player and media from `@videojs/react`. Import the installed skin component from `skin.tsx`, then render it around your media where you previously rendered the packaged skin.
</FrameworkCase>

<FrameworkCase frameworks={["html"]}>
The skin's files are under `components/videojs/<preset>`. Keep importing the player and media from `@videojs/html`. Import the installed `skin.ts` once, then paste the markup from `skin.html` inside your player around the media element.
</FrameworkCase>
...
To understand how skins fit into Video.js, read <DocsLink slug="guides/skins">Skins</DocsLink>. To create a control instead of editing one, follow <DocsLink slug="guides/build-your-own-component">Build your own component</DocsLink>.

## Related components

- <DocsLink slug="reference/components/video-skin">VideoSkin</DocsLink>

## Related guides

- <DocsLink slug="guides/skins">Skins</DocsLink>
