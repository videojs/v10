...
Wrap your control UI in <DocsLink slug="reference/components/controls">Controls</DocsLink>. It reads controls visibility from player state, so the whole control bar shows and hides as one unit.
</FrameworkCase>

<FrameworkCase frameworks={["html"]}>
Wrap your control UI in <DocsLink slug="reference/components/controls">`<media-controls>`</DocsLink>. It reads controls visibility from player state, so the whole control bar shows and hides as one unit.
</FrameworkCase>
...
The <DocsLink slug="reference/api/feature-controls">controls feature</DocsLink> tracks activity on the player container:
...
<FrameworkCase frameworks={["react"]}>
Set `visibility="always"` on <DocsLink slug="reference/components/controls">Controls</DocsLink>. The controls stay visible and report the user as active while the feature keeps tracking activity for anything else that needs it. The packaged audio skins use this mode.
</FrameworkCase>

<FrameworkCase frameworks={["html"]}>
Set `visibility="always"` on <DocsLink slug="reference/components/controls">`<media-controls>`</DocsLink>. The controls stay visible and report the user as active while the feature keeps tracking activity for anything else that needs it. The packaged audio skins use this mode.
</FrameworkCase>
...
Use the skin gestures (`tap` with the `toggleControls` action) rather than adding your own tap handlers next to the built-in behavior; competing handlers invert each other. See <DocsLink slug="guides/keyboard-shortcuts">Add keyboard shortcuts and gestures</DocsLink>.
...
- <DocsLink slug="reference/api/feature-controls">Controls feature</DocsLink>

## Related guides

- <DocsLink slug="guides/keyboard-shortcuts">Add keyboard shortcuts and gestures</DocsLink>
