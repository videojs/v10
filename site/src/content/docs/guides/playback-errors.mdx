...
These codes identify Video.js 8 APIs used with the Video.js 10 `video.js` package. Look up the code in the <DocsLink slug="reference/api/error-codes">error code reference</DocsLink> for its trigger and v10 equivalent. The player `error` state covered in this guide receives media playback errors after setup.
