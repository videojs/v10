...
- The provider resolves the locale: explicit `locale` first, then the nearest ancestor `lang` attribute, then `'en'`. It re-resolves when the page's `lang` changes.
- The provider lazy-loads the pack for the resolved locale automatically and keeps it in a layer above the shared registry (<DocsLink slug="reference/api/register-i18n">`registerI18n`</DocsLink>). Translations merge along a fallback chain that follows the [BCP 47](https://www.rfc-editor.org/rfc/rfc5646) parent tags — `es-MX` checks `es`, then `en` — so a regional pack only has to carry the text that differs from its parent.
- Each piece of text resolves from the first source that has it:
  1. Overrides supplied to the current player (the `translations` prop)
...
<FrameworkCase frameworks={["react"]}>
- English is built in; every other locale is a lazy-loaded pack. Server rendering needs an explicit `locale` since there's no DOM to resolve from.
- Locale tags are [BCP 47](https://www.rfc-editor.org/rfc/rfc5646) (`es`, `pt-BR`).
- Translation keys are namespaced strings (`buttons.play`, `errors.network`, `menu.quality`); the full list lives at <DocsLink slug="reference/api/translation-phrases">Translation phrases</DocsLink> and is typed, so typos in `registerI18n` objects fail in TypeScript. The translator itself accepts any string; a mistyped `t()` key falls back to its `default`.
...
Pass `translations` on <DocsLink slug="reference/api/i18n-provider">`I18nProvider`</DocsLink> to scope overrides to one subtree. This layer sits above the registry and lazy packs:
...
- <DocsLink slug="reference/api/i18n-provider" frameworks="react">I18nProvider</DocsLink>
- <DocsLink slug="reference/api/media-i18n" frameworks="html">media-i18n</DocsLink>
- <DocsLink slug="reference/api/media-text" frameworks="html">media-text</DocsLink>

## Related API

- <DocsLink slug="reference/api/register-i18n">registerI18n</DocsLink>
- <DocsLink slug="reference/api/translation-phrases">Translation phrases</DocsLink>
- <DocsLink slug="reference/api/use-translator" frameworks="react">useTranslator</DocsLink>
- <DocsLink slug="reference/api/use-locale" frameworks="react">useLocale</DocsLink>
- <DocsLink slug="reference/api/i18n-controller" frameworks="html">I18nController</DocsLink>
- <DocsLink slug="reference/api/get-i18n-translations">getI18nTranslations</DocsLink>

## Related guides

- <DocsLink slug="guides/captions">Show captions and subtitles</DocsLink>
- <DocsLink slug="guides/accessibility">Accessibility</DocsLink>
