...
- Translation keys are namespaced strings (`buttons.play`, `errors.network`, `menu.quality`); the full list lives at <DocsLink slug="reference/api/translation-phrases">Translation keys</DocsLink> and is typed, so typos in `registerI18n` objects fail in TypeScript. The translator itself accepts any string; a mistyped `t()` key falls back to its `default`.
...
- <DocsLink slug="reference/api/translation-phrases">Translation keys</DocsLink>
...
- <DocsLink slug="reference/api/i-18-n-controller" frameworks="html">I18nController</DocsLink>
