...
Quality selection applies to streaming sources. A progressive MP4 plays at one fixed quality and exposes no renditions; HLS and DASH streams offer several. See <DocsLink slug="guides/media-sources">Media sources</DocsLink>.
...
The <DocsLink slug="reference/api/feature-quality">quality feature</DocsLink> mirrors the media element's video renditions into player state:
...
<DocsLink slug="reference/api/use-quality-options">`useQualityOptions`</DocsLink> turns this state into ready-made menu options — labels like "1080p", an "Auto" entry, and `availability` — as the example above shows.

</FrameworkCase>
<FrameworkCase frameworks={["html"]}>

<DocsLink slug="reference/components/quality-radio-group">`<media-quality-radio-group>`</DocsLink> turns this state into a ready-made radio menu with resolution labels and an "Auto" entry, as the example above shows.
...
Access the player store through the <DocsLink slug="reference/api/player-controller">player controller</DocsLink> and call `selectVideoRendition` with a rendition `id` from `videoRenditionList`, or `'auto'` to restore adaptive selection.
...
- <DocsLink slug="reference/components/quality-radio-group">Quality radio group</DocsLink>
- <DocsLink slug="reference/components/menu">Menu</DocsLink>

## Related API

- <DocsLink slug="reference/api/feature-quality">Quality feature</DocsLink>
- <DocsLink slug="reference/api/use-quality-options" frameworks="react">useQualityOptions</DocsLink>
- <DocsLink slug="reference/components/hls-video">HlsVideo</DocsLink>
- <DocsLink slug="reference/components/hlsjs-video">HlsJsVideo</DocsLink>
- <DocsLink slug="reference/components/dash-video">DashVideo</DocsLink>
