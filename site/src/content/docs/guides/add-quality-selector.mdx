...
The <DocsLink slug="api/feature-quality">quality feature</DocsLink> mirrors the media element's video renditions into player state:
...
<DocsLink slug="api/use-quality-options">`useQualityOptions`</DocsLink> turns this state into ready-made menu options — labels like "1080p", an "Auto" entry, and `availability` — as shown in the demo above.
...
Access the player store through the <DocsLink slug="api/player-controller">player controller</DocsLink> and call `selectVideoRendition` with a rendition `id` from `videoRenditionList`, or `'auto'` to restore adaptive selection.
...
- <DocsLink slug="api/feature-quality">Quality feature</DocsLink>
- <DocsLink slug="api/use-quality-options">useQualityOptions</DocsLink>
