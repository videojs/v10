...
A skin bundle registers every UI element its skin is built from. The CDN publishes no per-component bundles, though. If you have <DocsLink slug="guides/customize-skins" anchor="eject-and-edit-a-skin">ejected a skin</DocsLink> and need to add new components, install from npm instead of from the CDN.
...
One bundle exists per locale pack, plus bare-language aliases such as `pt`. For custom strings, see <DocsLink slug="guides/internationalize-the-player">Internationalize the player</DocsLink>.
...
<DocsLink slug="guides/self-host-the-player">Self-host the player</DocsLink> shows both in use.
