...
- The provider lazy-loads the pack for the resolved locale automatically and keeps it in a layer above the shared registry (<DocsLink slug="api/register-i18n">`registerI18n`</DocsLink>). Translations merge along a fallback chain that follows the [BCP 47](https://www.rfc-editor.org/rfc/rfc5646) parent tags — `es-MX` checks `es`, then `en` — so a regional pack only has to carry the text that differs from its parent.
...
- Translation keys are namespaced strings (`buttons.play`, `errors.network`, `menu.quality`); the full list lives at <DocsLink slug="api/translation-phrases">Translation keys</DocsLink> and is typed, so typos in `registerI18n` objects fail in TypeScript. The translator itself accepts any string; a mistyped `t()` key falls back to its `default`.
...
Pass `translations` on <DocsLink slug="api/i18n-provider">`I18nProvider`</DocsLink> to scope overrides to one subtree. This layer sits above the registry and lazy packs:
...
- <DocsLink slug="api/i18n-provider">I18nProvider</DocsLink>
- <DocsLink slug="api/media-i18n">media-i18n</DocsLink>
- <DocsLink slug="api/media-text">media-text</DocsLink>

## Related API

- <DocsLink slug="api/register-i18n">registerI18n</DocsLink>
- <DocsLink slug="api/translation-phrases">Translation keys</DocsLink>
- <DocsLink slug="api/use-translator">useTranslator</DocsLink>
- <DocsLink slug="api/use-locale">useLocale</DocsLink>
- <DocsLink slug="api/get-i18n-translations">getI18nTranslations</DocsLink>
