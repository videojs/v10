...
| `rewind`, `fast-forward`, `seekTime` | The audio skins include 10-second skip buttons; the video skins do not. [Add the skin source to your project](#edit-skin-source), then add a <DocsLink slug="reference/components/seek-button">seek button</DocsLink>, which skips by its `seconds` value (default `30`; negative values seek backward). |
