...
Use the <DocsLink slug="reference/components/google-cast">GoogleCast extension</DocsLink> with a <DocsLink slug="reference/components/cast-button">CastButton</DocsLink>, and add an <DocsLink slug="reference/components/airplay-button">AirPlayButton</DocsLink> for Safari. The prebuilt skins already include both buttons; add `GoogleCast` next to a streaming media component to enable the configurable Cast route.
</FrameworkCase>

<FrameworkCase frameworks={["html"]}>
Use the <DocsLink slug="reference/components/google-cast">`<google-cast>` extension</DocsLink> with a <DocsLink slug="reference/components/cast-button">`<media-cast-button>`</DocsLink>, and add an <DocsLink slug="reference/components/airplay-button">`<media-airplay-button>`</DocsLink> for Safari. The prebuilt skins already include both buttons; add `<google-cast>` next to a streaming media component to enable the configurable Cast route.
</FrameworkCase>
...
<FrameworkCase frameworks={["react"]}>
The demo pairs the streaming media component with the `GoogleCast` extension. Without that extension, `CastButton` drives the browser's native Remote Playback API instead, which does not provide Video.js receiver or load-request configuration.
</FrameworkCase>
...
The <DocsLink slug="reference/api/feature-remote-playback">remote playback feature</DocsLink> gives AirPlay and Google Cast one state shape:
...
<FrameworkCase frameworks={["react"]}>
The Google Cast SDK loads only when the `GoogleCast` extension is present in a Chromium browser. Players without that extension and browsers that cannot cast do not load it.
</FrameworkCase>
...
<FrameworkCase frameworks={["react"]}>
Use `src` and `contentType` when the receiver should load a different source than the browser. `streamType` and `customData` let you describe that source and attach application-specific data. See the <DocsLink slug="reference/components/google-cast">GoogleCast reference</DocsLink> for the exact option types.
</FrameworkCase>

<FrameworkCase frameworks={["html"]}>
Use `src` and `contentType` when the receiver should load a different source than the browser. `streamType` and `customData` let you describe that source and attach application-specific data. See the <DocsLink slug="reference/components/google-cast">`<google-cast>` reference</DocsLink> for the exact option types and attribute-to-property mapping.
</FrameworkCase>
...
- <DocsLink slug="reference/components/cast-button">Cast button</DocsLink>
- <DocsLink slug="reference/components/airplay-button">AirPlay button</DocsLink>
- <DocsLink slug="reference/components/google-cast">GoogleCast</DocsLink>

## Related API

- <DocsLink slug="reference/api/feature-remote-playback">Remote playback feature</DocsLink>
