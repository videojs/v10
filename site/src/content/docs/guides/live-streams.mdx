...
The examples use HlsJsVideo, so install hls.js with the framework façade:
</FrameworkCase>
...
<FrameworkCase frameworks={["react"]}>
The examples use <DocsLink slug="reference/components/hlsjs-video">`HlsJsVideo`</DocsLink> for broad compatibility, but any streaming media component works here, including the smaller <DocsLink slug="reference/components/hls-video">`HlsVideo`</DocsLink>. Use `HlsVideo` when its supported feature set fits your stream.
</FrameworkCase>

<FrameworkCase frameworks={["html"]}>
The examples use <DocsLink slug="reference/components/hlsjs-video">`<hlsjs-video>`</DocsLink> for broad compatibility, but any streaming media component works here, including the smaller <DocsLink slug="reference/components/hls-video">`<hls-video>`</DocsLink>. Use `<hls-video>` when its supported feature set fits your stream.
</FrameworkCase>

The `liveVideoFeatures` bundle mirrors <DocsLink slug="guides/features">`videoFeatures`</DocsLink> but adds the live feature and drops playback rate (not meaningful for live), quality selection, and audio-track selection. The live presets also ship skins without a time slider, duration display, or current-time display.
...
The <DocsLink slug="reference/api/feature-time">time feature</DocsLink> adjusts for live playback: `duration` reports the live edge (the end of the seekable range) and keeps growing as the stream continues.
...
<FrameworkCase frameworks={["react"]}>
For streams with a DVR window, give users a <DocsLink slug="reference/components/time-slider">TimeSlider</DocsLink>. The live preset skins don't include one, so add it to your custom UI or <DocsLink slug="guides/customize-skins" anchor="style-skin-source">add the live skin source to your project</DocsLink>. The slider tracks the sliding window automatically because `duration` follows the live edge.
</FrameworkCase>

<FrameworkCase frameworks={["html"]}>
For streams with a DVR window, give users a <DocsLink slug="reference/components/time-slider">`<media-time-slider>`</DocsLink>. The live preset skins don't include one, so add it to your custom UI or <DocsLink slug="guides/customize-skins" anchor="style-skin-source">add the live skin source to your project</DocsLink>. The slider tracks the sliding window automatically because `duration` follows the live edge.
</FrameworkCase>
...
The preset `LiveVideoPlayer` comes with a fixed feature set. To change it, build the player yourself with <DocsLink slug="reference/api/create-player">`createPlayer`</DocsLink> — the escape hatch for custom feature sets:
...
<FrameworkCase frameworks={["react"]}>
`streamType` isn't `'live'`. Confirm the player's features include `streamTypeFeature` (`liveVideoFeatures` does not include it, so `streamType` reads as `undefined`), the source is a live manifest, and it plays through a live-capable media element such as <DocsLink slug="reference/components/hlsjs-video">HlsJsVideo</DocsLink>.
</FrameworkCase>

<FrameworkCase frameworks={["html"]}>
`streamType` isn't `'live'`. Confirm the player's features include `streamTypeFeature` (`liveVideoFeatures` does not include it, so `streamType` reads as `undefined`), the source is a live manifest, and it plays through a live-capable media element such as <DocsLink slug="reference/components/hlsjs-video">`<hlsjs-video>`</DocsLink>.
</FrameworkCase>
...
- <DocsLink slug="reference/components/time-slider">Time slider</DocsLink>
- <DocsLink slug="reference/components/buffering-indicator">Buffering indicator</DocsLink>

## Related API

- <DocsLink slug="reference/api/feature-live">Live feature</DocsLink>
- <DocsLink slug="reference/api/feature-stream-type">Stream type feature</DocsLink>
- <DocsLink slug="reference/api/feature-time">Time feature</DocsLink>
- <DocsLink slug="reference/components/hlsjs-video">HlsJsVideo</DocsLink>
