...
When you add the skin files, you also take on their CSS. The packaged skins ship styles for everything above; a bare layout renders unstyled until you bring those along, so the copied files include the skin's stylesheet next to its markup.

## Plugins

v10 has no plugin system. There's no `videojs.registerPlugin`, no `player.myPlugin()`, and no plugin lifecycle.

That's a deliberate choice rather than a missing feature. v8 plugins existed largely because there was no other way in: to add a control, change a behavior, or support a format, you had to reach into player internals through the one door the plugin API provided. v10 gives you the front door instead: components you can add, skin files you can change, and media elements you can swap. Most of what plugins did is now ordinary composition.
...
- **UI additions** (extra buttons, overlays, custom control bars). Rebuild them by editing the skin files in your project, or with <DocsLink slug="guides/build-your-own-component">your own component</DocsLink>. This is usually less code than the plugin was.
