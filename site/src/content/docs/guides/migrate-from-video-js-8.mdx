...
| the title in `TitleBar` | `title` on `VideoPlayer`, rendered with the Title component |
</FrameworkCase>
...
<FrameworkCase frameworks={["react"]}>
The packaged video and live-video skins already include the <DocsLink slug="reference/components/title">Title</DocsLink> component, so setting `title` on the player displays it. Audio skins do not include a title. Add `Title` where you want it in a custom layout or skin source you added to your project. For poster rendering options — including placeholders and custom image elements — see <DocsLink slug="guides/poster">Add a poster and loading placeholder</DocsLink> and the <DocsLink slug="reference/components/poster">Poster</DocsLink> reference.
</FrameworkCase>

<FrameworkCase frameworks={["html"]}>
The packaged video and live-video skins already include the <DocsLink slug="reference/components/title">`<media-title>`</DocsLink> element, so setting `content-title` on the player displays it. Audio skins do not include a title. Add `<media-title>` where you want it in a custom layout or skin source you added to your project. For poster rendering options — including placeholders and custom image elements — see <DocsLink slug="guides/poster">Add a poster and loading placeholder</DocsLink> and the <DocsLink slug="reference/components/poster">`<media-poster>`</DocsLink> reference.
</FrameworkCase>
...
| `ModalDialog` | compose a <DocsLink slug="reference/components/dialog">dialog</DocsLink>; your application controls media playback when it opens and closes |
...
Package-manager installations also need the matching media package. The default HLS component shown below uses
...
- **UI additions** (extra buttons, overlays, custom control bars). Rebuild them by editing the skin source in your project, or with <DocsLink slug="guides/build-your-own-component">your own component</DocsLink>. This is usually less code than the plugin was.
...
| `player.src({ src, type })` | set `src` on the media or replace the media component (`Video`, `HlsJsVideo`, `DashVideo`, etc) |
...
| `player.dispose()` | unmount the component |
| `player.tech()` | the media component's public `engine` escape hatch, where one exists; reach it through `useMedia()` |
| `videojs.getPlayer(id)` | call `usePlayer` from a component inside the player |
</FrameworkCase>
