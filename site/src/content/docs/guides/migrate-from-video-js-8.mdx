...
`VideoPlayer` is the piece that owns the state. To read that state, the preset ships a matching <DocsLink slug="api/use-player">`usePlayer`</DocsLink> hook. There's no ref to manage and no disposal to remember.
...
Use <DocsLink slug="api/player-controller">`PlayerController`</DocsLink> inside a custom element. Give it a selector for the feature you care about and it keeps your element in sync as that state changes:
...
Put standard media event props such as `onPlay`, `onTimeUpdate`, `onEnded`, and `onError` on `Video` or your chosen media component. Read player state through the preset's <DocsLink slug="api/use-player">`usePlayer`</DocsLink> hook, typed to that preset's features:
...
<FrameworkCase frameworks={["react"]}>
To override individual strings, use <DocsLink slug="api/i18n-provider">`I18nProvider`</DocsLink> when you want them scoped to a subtree:
...
<FrameworkCase frameworks={["html"]}>
That's what <DocsLink slug="api/html-create-player">`createPlayer`</DocsLink> is for. Hand it a feature list and you get back the mixins to define your own player element.
</FrameworkCase>

<FrameworkCase frameworks={["react"]}>
That's what <DocsLink slug="api/create-player">`createPlayer`</DocsLink> is for. Hand it a feature list and you get back a `Player` component and a `usePlayer` hook, the same pair every preset is built from.
</FrameworkCase>
