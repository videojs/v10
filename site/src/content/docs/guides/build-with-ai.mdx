...
import markdownNegotiation from '@/assets/docs/guides/build-with-ai/markdown-negotiation.png';
...
1. **Copy page button**: each page has a "Copy page" button beside its breadcrumbs. Grab the pages relevant to your task and paste them in. The button looks something like this...
...
3. **Add `.md` to any URL**: append `.md` to any doc URL to get its markdown directly. For example, the PlayButton reference is available at [`videojs.org/docs/framework/react/components/play-button.md`](https://videojs.org/docs/framework/react/components/play-button.md).
</FrameworkCase>
...
2. **`Accept: text/markdown` header**: AI tools that send this header when fetching a Video.js doc URL will receive markdown instead of HTML.
   ...
   <ContentWidth><Img src={markdownNegotiation} alt="Fetching a doc URL returns 188.7KB of HTML; fetching the same URL with the Accept: text/markdown header returns 3.4KB of markdown" class="w-full max-w-md" /></ContentWidth>
3. **Add `.md` to any URL**: append `.md` to any doc URL to get its markdown directly. For example, the PlayButton reference is available at [`videojs.org/docs/framework/html/components/play-button.md`](https://videojs.org/docs/framework/html/components/play-button.md).
</FrameworkCase>
