...
### How the skill finds current instructions and docs

The skill uses documentation in this order:

1. **Version-matched installation instructions.** `npx @videojs/react agents init` and `npx @videojs/html agents init` use the installed player package to list valid choices. Adding flags prints one complete installation plan. The command only returns instructions; it does not prompt, install packages, or write files. Add `@latest` before `agents init` when the package is not installed yet. For a CDN page, use the version in its script URL instead of `@latest`.
...
3. **Live docs.** When the agent cannot run commands, the skill starts at [`videojs.org/docs/guides/installation.md`](https://videojs.org/docs/guides/installation.md) and requests the selected installation page with query parameters as Markdown. For docs beyond installation, it reads the framework index on videojs.org and opens only the pages the task needs.
...
The old `npx @videojs/cli docs` workflow is deprecated. Use the `agents init` command from the React or HTML package so the instruction renderer ships with the player version.
