...
The <DocsLink slug="api/feature-fullscreen">fullscreen feature</DocsLink> tracks fullscreen state and provides the actions:
...
Because the feature list changes, build the player yourself with <DocsLink slug="api/create-player">`createPlayer`</DocsLink> — the escape hatch for custom feature sets:
...
Compose a player with `orientationLockFeature` in its feature list through <DocsLink slug="api/html-create-player">`createPlayer`</DocsLink>; the prebuilt `<video-player>` element doesn't include it.

</FrameworkCase>

To lock to something other than `landscape`, set `orientationLockType` on the player (the `orientation-lock-type` attribute in HTML). See the <DocsLink slug="api/feature-orientation-lock">orientation lock feature</DocsLink> reference for details.
...
- <DocsLink slug="api/feature-fullscreen">Fullscreen feature</DocsLink>
- <DocsLink slug="api/feature-orientation-lock">Orientation lock feature</DocsLink>
