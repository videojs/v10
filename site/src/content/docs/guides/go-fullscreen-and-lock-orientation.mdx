...
Add a <DocsLink slug="components/fullscreen-button">FullscreenButton</DocsLink> inside the player container. It requests fullscreen on the container, so everything inside it — controls, captions, overlays — stays visible in the fullscreen presentation.
...
The <DocsLink slug="components/feature-fullscreen">fullscreen feature</DocsLink> tracks fullscreen state and provides the actions:
...
Because the feature list changes, build the player yourself with <DocsLink slug="components/create-player">`createPlayer`</DocsLink> — the escape hatch for custom feature sets:
...
Compose a player with `orientationLockFeature` in its feature list through <DocsLink slug="components/html-create-player">`createPlayer`</DocsLink>; the prebuilt `<video-player>` element doesn't include it.

</FrameworkCase>

To lock to something other than `landscape`, set `orientationLockType` on the player (the `orientation-lock-type` attribute in HTML). See the <DocsLink slug="components/feature-orientation-lock">orientation lock feature</DocsLink> reference for details.
...
- <DocsLink slug="components/fullscreen-button">Fullscreen button</DocsLink>
- <DocsLink slug="components/player-container">Player container</DocsLink>

## Related API

- <DocsLink slug="components/feature-fullscreen">Fullscreen feature</DocsLink>
- <DocsLink slug="components/feature-orientation-lock">Orientation lock feature</DocsLink>
