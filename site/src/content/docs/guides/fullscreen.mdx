...
Add a <DocsLink slug="reference/components/fullscreen-button">FullscreenButton</DocsLink> inside the player container. It requests fullscreen on the container, so everything inside it — controls, captions, overlays — stays visible in the fullscreen presentation.
</FrameworkCase>

<FrameworkCase frameworks={["html"]}>
Add a <DocsLink slug="reference/components/fullscreen-button">`<media-fullscreen-button>`</DocsLink> inside the player container. It requests fullscreen on the container, so everything inside it — controls, captions, overlays — stays visible in the fullscreen presentation.
</FrameworkCase>
...
The <DocsLink slug="reference/api/feature-fullscreen">fullscreen feature</DocsLink> tracks fullscreen state and provides the actions:
...
Because the feature list changes, build the player yourself with <DocsLink slug="reference/api/create-player">`createPlayer`</DocsLink> — the escape hatch for custom feature sets:
...
Add `orientationLockFeature` to lock screen orientation while the player is fullscreen. It isn't part of `videoFeatures`, so the preset `<video-player>` doesn't include it. It locks to `landscape` by default and unlocks on exit.

Compose a player with `orientationLockFeature` in its feature list through <DocsLink slug="reference/api/html-create-player">`createPlayer`</DocsLink>; the prebuilt `<video-player>` element doesn't include it.

</FrameworkCase>

<FrameworkCase frameworks={["react"]}>
To lock to something other than `landscape`, set `orientationLockType` on the player. See the <DocsLink slug="reference/api/feature-orientation-lock">orientation lock feature</DocsLink> reference for details.
</FrameworkCase>

<FrameworkCase frameworks={["html"]}>
To lock to something other than `landscape`, set the `orientation-lock-type` attribute on the player. See the <DocsLink slug="reference/api/feature-orientation-lock">orientation lock feature</DocsLink> reference for details.
</FrameworkCase>
...
Use the `<media-gesture>` element — the default skins ship this exact binding. Keep the button as the accessible path; gestures are a pointer-only shortcut.
...
`region="center"` keeps the gesture off the left and right thirds, which the default skins reserve for double-tap seeking. See <DocsLink slug="guides/keyboard-shortcuts">Add keyboard shortcuts and gestures</DocsLink> for the full gesture surface.
...
- <DocsLink slug="reference/components/fullscreen-button">Fullscreen button</DocsLink>
- <DocsLink slug="reference/components/player-container">Player container</DocsLink>

## Related API

- <DocsLink slug="reference/api/feature-fullscreen">Fullscreen feature</DocsLink>
- <DocsLink slug="reference/api/feature-orientation-lock">Orientation lock feature</DocsLink>

## Related guides

- <DocsLink slug="guides/picture-in-picture">Use picture-in-picture</DocsLink>
