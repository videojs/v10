...
Follow the <DocsLink slug="guides/installation">installation guide</DocsLink> first. Keep its `@videojs/html` imports and player markup in your Svelte component, then apply the Svelte-specific patterns below.
...
`:defined` applies the state style only after the browser loads the element. These selectors matter when you add the skin source to your project or compose controls yourself. Styles cannot reach controls inside a packaged skin; use its CSS custom properties, or <DocsLink slug="guides/customize-skins" anchor="style-skin-source">add the skin source to your project</DocsLink> to change its markup and CSS.
...
- <DocsLink slug="guides/installation-svelte">Installation</DocsLink>
