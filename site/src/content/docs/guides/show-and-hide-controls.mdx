...
Wrap your control UI in <DocsLink slug="components/controls">Controls</DocsLink>. It reads controls visibility from player state, so the whole control bar shows and hides as one unit.
...
The <DocsLink slug="api/feature-controls">controls feature</DocsLink> tracks activity on the player container:
...
Set `visibility="always"` on <DocsLink slug="components/controls">Controls</DocsLink>. The controls stay visible and report the user as active while the feature keeps tracking activity for anything else that needs it. The packaged audio skins use this mode.
...
- <DocsLink slug="api/feature-controls">Controls feature</DocsLink>
