...
The <DocsLink slug="api/feature-time">time feature</DocsLink> adjusts for live playback: `duration` reports the live edge (the end of the seekable range) and keeps growing as the stream continues.
...
The preset `LiveVideoPlayer` comes with a fixed feature set. To change it, build the player yourself with <DocsLink slug="api/create-player">`createPlayer`</DocsLink> — the escape hatch for custom feature sets:
...
- <DocsLink slug="api/feature-live">Live feature</DocsLink>
- <DocsLink slug="api/feature-stream-type">Stream type feature</DocsLink>
- <DocsLink slug="api/feature-time">Time feature</DocsLink>
