...
UI components can go anywhere inside a <DocsLink slug="reference/components/player">`<Player>`</DocsLink>.
...
<DocsLinkCard slug="reference/components/player-container">Read more about `<Container>`</DocsLinkCard>
</FrameworkCase>
...
UI components can go anywhere inside a <DocsLink slug="reference/components/player">`<video-player>`</DocsLink>.
...
<DocsLinkCard slug="reference/components/player-container">Read more about `<media-container>`</DocsLinkCard>
</FrameworkCase>
...
```tsx
<PlayButton className={(state) => (state.paused ? 'btn btn--paused' : 'btn btn--playing')} />
```
...
button:not([data-paused]) .pause-icon {
  display: inline;
}
```
