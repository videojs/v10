...
Packaged HTML skins inline their CSS into JavaScript, so <DocsLink slug="guides/customize-skins" anchor="eject-and-edit-a-skin">eject the skin</DocsLink> first; the ejected `skin.css` is a file in your project. CDN bundles inline their CSS too, so a CDN player needs <DocsLink slug="guides/self-hosting">self-hosting</DocsLink> plus an eject.
