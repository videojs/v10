...
To reach browsers below the floor, recompile the skin stylesheet with fallbacks. Only a stylesheet your build processes can be recompiled: React skins are imported CSS files, HTML skins inline their CSS into JavaScript, so <DocsLink slug="guides/customize-skins" anchor="eject-and-edit-a-skin">eject the skin</DocsLink> first, and CDN bundles need <DocsLink slug="guides/self-hosting">self-hosting</DocsLink> plus an eject.
