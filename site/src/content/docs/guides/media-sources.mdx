...
| An extension's own settings | that extension's props | <DocsLink slug="guides/mux-data">Mux Data</DocsLink>, <DocsLink slug="reference/components/google-cast">GoogleCast</DocsLink> |
</FrameworkCase>
...
| How Video.js plays it | `source`, at the top level | `preferPlayback`, `maxAutoResolution`, `capRenditionToPlayerSize` |
| How a specific engine behaves | `source.engine`, under that engine's name | hls.js's `maxBufferLength`, in `source.engine.hlsJs` |
| An extension's own settings | that extension's attributes and properties | <DocsLink slug="guides/mux-data">Mux Data</DocsLink>, <DocsLink slug="reference/components/google-cast">`<google-cast>`</DocsLink> |
</FrameworkCase>
...
|---|---|---|
| `engine.hlsJs` | <DocsLink slug="reference/components/hlsjs-video">HlsJsVideo</DocsLink>, <DocsLink slug="reference/components/mux-video">MuxVideo</DocsLink>, <DocsLink slug="reference/components/mux-audio">MuxAudio</DocsLink> | an [hls.js config](https://github.com/video-dev/hls.js/blob/master/docs/API.md#fine-tuning) |
| `engine.nativeHls` | <DocsLink slug="reference/components/native-hls-video">NativeHlsVideo</DocsLink>, and the three above whenever the browser plays the manifest | options for the browser's own HLS support (today, [DRM](#licensing-one-engine-differently)) |
| `engine.dashJs` | <DocsLink slug="reference/components/dash-video">DashVideo</DocsLink> | [dash.js settings](https://cdn.dashjs.org/latest/jsdoc/module-Settings.html) |
| `engine.vimeo` | <DocsLink slug="reference/components/vimeo-video">VimeoVideo</DocsLink> | [Vimeo embed parameters](https://developer.vimeo.com/player/sdk/embed) |
| `engine.youtube` | <DocsLink slug="reference/components/youtube-video">YouTubeVideo</DocsLink> | [YouTube player parameters](https://developer.chrome.com/docs/youtube/iframe_api_reference#Parameters) |
| `engine.cloudflare` | <DocsLink slug="reference/components/cloudflare-video">CloudflareVideo</DocsLink> | [Stream player parameters](https://developers.cloudflare.com/stream/viewing-videos/using-the-stream-player/) |
| `engine.spotify` | <DocsLink slug="reference/components/spotify-audio">SpotifyAudio</DocsLink> | [Spotify embed options](https://developer.spotify.com/documentation/embeds) |
| `engine.tiktok` | <DocsLink slug="reference/components/tiktok-video">TikTokVideo</DocsLink> | [TikTok player parameters](https://developers.tiktok.com/doc/embed-player) |
| `engine.twitch` | <DocsLink slug="reference/components/twitch-video">TwitchVideo</DocsLink> | [Twitch embed parameters](https://dev.twitch.tv/docs/embed/video-and-clips/) |
</FrameworkCase>

<FrameworkCase frameworks={["html"]}>
| Key | Elements that read it | It holds |
|---|---|---|
| `engine.hlsJs` | <DocsLink slug="reference/components/hlsjs-video">`<hlsjs-video>`</DocsLink>, <DocsLink slug="reference/components/mux-video">`<mux-video>`</DocsLink>, <DocsLink slug="reference/components/mux-audio">`<mux-audio>`</DocsLink> | an [hls.js config](https://github.com/video-dev/hls.js/blob/master/docs/API.md#fine-tuning) |
| `engine.nativeHls` | <DocsLink slug="reference/components/native-hls-video">`<native-hls-video>`</DocsLink>, and the three above whenever the browser plays the manifest | options for the browser's own HLS support (today, [DRM](#licensing-one-engine-differently)) |
| `engine.dashJs` | <DocsLink slug="reference/components/dash-video">`<dash-video>`</DocsLink> | [dash.js settings](https://cdn.dashjs.org/latest/jsdoc/module-Settings.html) |
| `engine.vimeo` | <DocsLink slug="reference/components/vimeo-video">`<vimeo-video>`</DocsLink> | [Vimeo embed parameters](https://developer.vimeo.com/player/sdk/embed) |
| `engine.youtube` | <DocsLink slug="reference/components/youtube-video">`<youtube-video>`</DocsLink> | [YouTube player parameters](https://developer.chrome.com/docs/youtube/iframe_api_reference#Parameters) |
| `engine.cloudflare` | <DocsLink slug="reference/components/cloudflare-video">`<cloudflare-video>`</DocsLink> | [Stream player parameters](https://developers.cloudflare.com/stream/viewing-videos/using-the-stream-player/) |
| `engine.spotify` | <DocsLink slug="reference/components/spotify-audio">`<spotify-audio>`</DocsLink> | [Spotify embed options](https://developer.spotify.com/documentation/embeds) |
| `engine.tiktok` | <DocsLink slug="reference/components/tiktok-video">`<tiktok-video>`</DocsLink> | [TikTok player parameters](https://developers.tiktok.com/doc/embed-player) |
| `engine.twitch` | <DocsLink slug="reference/components/twitch-video">`<twitch-video>`</DocsLink> | [Twitch embed parameters](https://dev.twitch.tv/docs/embed/video-and-clips/) |
</FrameworkCase>
...
<FrameworkCase frameworks={["react"]}>
So a source naming only Widevine plays through hls.js and cannot play on the native path — <DocsLink slug="reference/components/native-hls-video">NativeHlsVideo</DocsLink>, or any HLS element the browser ends up playing the manifest for. Video.js says so in development, and encrypted media that gets there with no `com.apple.fps` license server fails with a `MEDIA_ERR_ENCRYPTED` error rather than hanging.
</FrameworkCase>

<FrameworkCase frameworks={["html"]}>
So a source naming only Widevine plays through hls.js and cannot play on the native path — <DocsLink slug="reference/components/native-hls-video">`<native-hls-video>`</DocsLink>, or any HLS element the browser ends up playing the manifest for. Video.js says so in development, and encrypted media that gets there with no `com.apple.fps` license server fails with a `MEDIA_ERR_ENCRYPTED` error rather than hanging.
</FrameworkCase>
...
<FrameworkCase frameworks={["react"]}>
<DocsLink slug="reference/components/mux-video">MuxVideo</DocsLink> and <DocsLink slug="reference/components/mux-audio">MuxAudio</DocsLink> identify a source by `playbackId` rather than a URL, and derive `src` from it. Everything else works the same, engine options included:
</FrameworkCase>

<FrameworkCase frameworks={["html"]}>
<DocsLink slug="reference/components/mux-video">`<mux-video>`</DocsLink> and <DocsLink slug="reference/components/mux-audio">`<mux-audio>`</DocsLink> identify a source by `playbackId` rather than a URL, and derive `src` from it. Everything else works the same, engine options included:
</FrameworkCase>
...
<FrameworkCase frameworks={["react"]}>
<DocsLink slug="reference/components/hls-video">HlsVideo</DocsLink> and <DocsLink slug="reference/components/hls-audio">HlsAudio</DocsLink> take `src` and the usual media props but expose no `source`. They run on our own playback engine, which does not accept configuration from the element yet.
</FrameworkCase>

<FrameworkCase frameworks={["html"]}>
<DocsLink slug="reference/components/hls-video">`<hls-video>`</DocsLink> and <DocsLink slug="reference/components/hls-audio">`<hls-audio>`</DocsLink> take `src` and the usual media attributes but expose no `source`. They run on our own playback engine, which does not accept configuration from the element yet.
</FrameworkCase>
...
| `config.muxData` | the <DocsLink slug="guides/mux-data">Mux Data</DocsLink> extension's own props |
| `config.googleCast` | the <DocsLink slug="reference/components/google-cast">GoogleCast</DocsLink> extension's own props |
...
| `config.hlsJs` | `source.engine.hlsJs` |
| `config.dashJs` | `source.engine.dashJs` |
| `config.muxData` | the <DocsLink slug="guides/mux-data">Mux Data</DocsLink> extension's own attributes and properties |
| `config.googleCast` | the <DocsLink slug="reference/components/google-cast">`<google-cast>`</DocsLink> extension's own attributes and properties |
| `config` on `<vimeo-video>` | `source.engine.vimeo` |
</FrameworkCase>
...
<FrameworkCase frameworks={["react"]}>
`HlsVideo` and `HlsAudio` accepted a `config` that nothing read. Remove it — those components never applied it.
</FrameworkCase>
...
<FrameworkCase frameworks={["react"]}>
<DocsLinkCard slug="reference/components/hlsjs-video">HlsJsVideo element reference</DocsLinkCard>
<DocsLinkCard slug="reference/components/mux-video">MuxVideo element reference</DocsLinkCard>
<DocsLinkCard slug="reference/components/dash-video">DashVideo element reference</DocsLinkCard>
</FrameworkCase>

<FrameworkCase frameworks={["html"]}>
<DocsLinkCard slug="reference/components/hlsjs-video">hlsjs-video element reference</DocsLinkCard>
<DocsLinkCard slug="reference/components/mux-video">mux-video element reference</DocsLinkCard>
<DocsLinkCard slug="reference/components/dash-video">dash-video element reference</DocsLinkCard>
</FrameworkCase>
