...
<FrameworkCase frameworks={["react"]}>
<DocsLink slug="reference/components/hls-video">HlsVideo</DocsLink> and <DocsLink slug="reference/components/hls-audio">HlsAudio</DocsLink> take `src` or `source`, and the usual media props.
...
On HlsVideo, `drm` licenses FairPlay, Widevine, and PlayReady, taking the same license servers as [DRM protected sources](#drm-protected-sources). HlsAudio accepts `drm` but ignores it, so one source object serves both.
</FrameworkCase>

<FrameworkCase frameworks={["html"]}>
<DocsLink slug="reference/components/hls-video">`<hls-video>`</DocsLink> and <DocsLink slug="reference/components/hls-audio">`<hls-audio>`</DocsLink> take `src` or `source`, and the usual media attributes.
