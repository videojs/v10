...
Pass `selectRemotePlayback` to <DocsLink slug="components/use-player">`usePlayer`</DocsLink> to subscribe to remote playback state. Returns `undefined` if the remote playback feature is not configured.
</FrameworkCase>

<FrameworkCase frameworks={["html"]}>
Pass `selectRemotePlayback` to <DocsLink slug="components/player-controller">`PlayerController`</DocsLink> to subscribe to remote playback state. Returns `undefined` if the remote playback feature is not configured.
</FrameworkCase>
