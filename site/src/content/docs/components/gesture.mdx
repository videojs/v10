...
`Gesture` is a behavior component: it renders no visible interface. Place it inside a <DocsLink slug="components/player-container">Container</DocsLink> to map a `tap` or `doubletap` gesture to a player action. A tap must use the primary pointer and complete within 250 milliseconds. When a matching double-tap gesture exists, the first tap waits briefly to see whether a second tap follows.
