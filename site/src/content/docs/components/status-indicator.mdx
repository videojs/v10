...
`StatusIndicator` displays feedback for actions emitted by a <DocsLink slug="components/hotkey">`Hotkey`</DocsLink> or <DocsLink slug="components/gesture">`Gesture`</DocsLink> in the same player `Container`. It does not react to buttons, direct player-store changes, or media state changes on their own.
...
`StatusIndicator` is visual feedback and does not create a live region. Keep every action available through keyboard-operable controls, and pair the player with <DocsLink slug="components/status-announcer">`StatusAnnouncer`</DocsLink> when state changes should be announced to screen readers. Do not make `StatusIndicator.Value` a live region.
