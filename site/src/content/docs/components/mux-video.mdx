...
The storyboard lives on a Mux domain, so MuxVideo has to be CORS-enabled for that cross-origin `<track>` to load at all. <DocsLink slug="components/thumbnail">Thumbnail</DocsLink> then fetches the sprite sheets its cues point at the same way:
