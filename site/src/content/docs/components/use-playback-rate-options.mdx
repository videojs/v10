...
`usePlaybackRateOptions` returns the current playback rate, the available options, and `setRate` / `setValue` callbacks for wiring rate selection into a menu. It returns `null` when the <DocsLink slug="components/feature-playback-rate">playback rate feature</DocsLink> is not configured.
...
See <DocsLink slug="components/playback-rate-radio-group">PlaybackRateRadioGroup</DocsLink> for the component-level pattern, and <DocsLink slug="components/menu">Menu</DocsLink> for a complete settings menu example.
