...
The built-in selectors (<DocsLink slug="components/feature-playback">`selectPlayback`</DocsLink>, <DocsLink slug="components/feature-buffer">`selectBuffer`</DocsLink>, etc.) are all created with `createSelector`. Use it to create selectors for custom slices.
...
Pass selectors to <DocsLink slug="components/use-player">`usePlayer`</DocsLink> or <DocsLink slug="components/use-store">`useStore`</DocsLink> for reactive subscriptions.
</FrameworkCase>

<FrameworkCase frameworks={["html"]}>
Pass selectors to <DocsLink slug="components/player-controller">`PlayerController`</DocsLink> or <DocsLink slug="components/store-controller">`StoreController`</DocsLink> for reactive subscriptions.
</FrameworkCase>
