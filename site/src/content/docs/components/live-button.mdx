...
The button combines three player features: <DocsLink slug="components/feature-live">live</DocsLink> for `liveEdgeStart` and `targetLiveWindow`, <DocsLink slug="components/feature-time">time</DocsLink> for `currentTime` and the `seek()` action, and <DocsLink slug="components/feature-buffer">buffer</DocsLink> for `seekable`. All three are included in the `liveVideoFeatures` and `liveAudioFeatures` presets, and the packaged live skins include the button.
