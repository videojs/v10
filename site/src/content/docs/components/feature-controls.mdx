...
Pass `selectControls` to <DocsLink slug="components/use-player">`usePlayer`</DocsLink> to subscribe to controls state. Returns `undefined` if the controls feature is not configured.
</FrameworkCase>

<FrameworkCase frameworks={["html"]}>
Pass `selectControls` to <DocsLink slug="components/player-controller">`PlayerController`</DocsLink> to subscribe to controls state. Returns `undefined` if the controls feature is not configured.
</FrameworkCase>
