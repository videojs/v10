...
Use <DocsLink slug="components/alert-dialog">`AlertDialog`</DocsLink> for urgent messages that require acknowledgement. <DocsLink slug="components/error-dialog">`ErrorDialog`</DocsLink> is the player-error specialization. Both use the same dialog behavior.
