...
|                | `SnapshotController`           | <DocsLink slug="components/store-controller">`StoreController`</DocsLink> |
| -------------- | ------------------------------ | ------------------------------------------------------------------------ |
...
<DocsLink slug="components/store-controller">`StoreController`</DocsLink> is
...
`SnapshotController` subscribes on `hostConnected()` and unsubscribes on `hostDisconnected()`. The React equivalent is <DocsLink slug="components/use-snapshot">`useSnapshot`</DocsLink>.
