...
UI Components can go anywhere inside a <DocsLink slug="reference/player-provider">`<Player.provider>`</DocsLink>. Unlike your media, your UI components don't need to be inside the <DocsLink slug="reference/player-container">`<Player.Container>`</DocsLink> (though components inside the container will go fullscreen with the video.)
</FrameworkCase>

<FrameworkCase frameworks={["html"]}>
UI Components can go anywhere inside a <DocsLink slug="reference/player-provider">`<video-player>`</DocsLink>. Unlike your media, your UI components don't need to be inside the <DocsLink slug="reference/player-container">`<player-container>`</DocsLink> (though components inside the container will go fullscreen with the video.)
</FrameworkCase>
