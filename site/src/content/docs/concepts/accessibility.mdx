...
Announcements track confirmed state from the player store, not intent, so routine time updates never reach the live region. Volume and seek messages are debounced and coalesced into a single utterance, and the player stays quiet while one of its sliders has focus, since the slider already announces its own value. Store-backed state is covered for you; a custom control that tracks its own state needs its own live region.
