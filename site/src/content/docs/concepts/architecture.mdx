...
The state available in the store is determined by which features your player includes. Set up your player's features with a preset.

<DocsLinkCard slug="concepts/presets">Learn more about presets</DocsLinkCard>

## 2. User interface

You can approach UI in Video.js from a few different levels: use a prebuilt Skin, build your own UI from our components, or even build your own components.
...
You might consider <DocsLink slug="how-to/customize-skins">ejecting a skin</DocsLink> and using its pre-styled components as a starting point.

<DocsLinkCard slug="concepts/ui-components">Learn more about UI components</DocsLinkCard>

### Build your own UI components

You might not need to build your own UI component. Our components are unstyled primitives that offer built-in accessibility and behavior. However, if you notice that we don't support something (yet!), you can access player state directly.
