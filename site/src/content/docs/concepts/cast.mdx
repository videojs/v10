...
The Remote Playback feature reports availability as `'available'` (a device is on the network), `'unavailable'` (no device found), or `'unsupported'` (the browser can't cast). A <DocsLink slug="reference/cast-button">CastButton</DocsLink> turns this into presentation for you: it hides itself when Cast is unsupported and shows a disabled state when Cast is supported but no device is reachable. See its reference for the data attributes and styling hooks.
