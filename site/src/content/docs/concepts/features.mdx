...
Components that depend on availability (like <DocsLink slug="reference/pip-button">`PiPButton`</DocsLink> and <DocsLink slug="reference/fullscreen-button">`FullscreenButton`</DocsLink>) expose `data-available` and `data-availability` attributes. 

<FrameworkCase frameworks={["html"]}>
The default skins automatically hide unavailable buttons. For custom skins, you can hide them with CSS:
```css
media-pip-button:not([data-available]) {
  display: none;
}
```
</FrameworkCase>
...
<FrameworkCase frameworks={["react"]}>
In React, the button returns `null` when unavailable — no CSS needed.
</FrameworkCase>
