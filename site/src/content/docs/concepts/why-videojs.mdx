...
**Streaming formats.** Most production video isn't an MP4. It's HLS or DASH, broken into segments and adapted to each viewer's bandwidth. Safari plays HLS natively. Chrome and Firefox don't, and no major browser plays DASH. A player library closes the gap so the same `src` works everywhere.
...
<FrameworkCase frameworks={["react"]}>
```tsx
import { HlsVideo } from '@videojs/react/media/hls-video'; // [!code ++]

<Player.Provider>
  <VideoSkin>
    <HlsVideo src="https://stream.mux.com/.../stream.m3u8" /> {/* [!code focus] */}
  </VideoSkin>
</Player.Provider>
```
</FrameworkCase>
<FrameworkCase frameworks={["html"]}>
```html
<script type="module" src="@videojs/html/media/hls-video"></script> <!-- [!code ++] -->

<video-player>
  <video-skin>
    <hls-video src="https://stream.mux.com/.../stream.m3u8"></hls-video> <!-- [!code focus] -->
  </video-skin>
</video-player>
```
</FrameworkCase>
