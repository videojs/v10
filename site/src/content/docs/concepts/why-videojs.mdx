...
Most player libraries draw a line between "configurable through props" and "fork the source." Cross the line and you're on your own. Video.js lets you install any skin's source and walk away with it: components in your framework's language that you can read and change. The skin you ship is yours; the rest of the player keeps working underneath it.

We don't want to cram every possible player into one configuration API. Packaged skins cover common visual changes. For everything else, own the source early and compose the UI you need. The Shadcn CLI copies any skin from the Video.js registry into your project.
