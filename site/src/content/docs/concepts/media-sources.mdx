...
| An extension's own settings | that extension's props | <DocsLink slug="guides/monitor-playback-with-mux-data">Mux Data</DocsLink>, <DocsLink slug="reference/components/google-cast">GoogleCast</DocsLink> |
...
<DocsLink slug="reference/components/hls-video">HlsVideo</DocsLink> and <DocsLink slug="reference/components/hls-audio">HlsAudio</DocsLink> take `src` and the usual media attributes but expose no `source`. They run on our own playback engine, which does not accept configuration from the element yet.
...
| `config.muxData` | the <DocsLink slug="guides/monitor-playback-with-mux-data">Mux Data</DocsLink> extension's own props |
