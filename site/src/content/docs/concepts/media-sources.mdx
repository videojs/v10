...
`source.poster` is only data. Nothing applies it to the media, so build the URL with `createMuxPosterURL` and use it wherever you need a still:

```ts
import { createMuxPosterURL } from '@videojs/media/dom/mux';

const url = createMuxPosterURL(video.source);
```

Passing a second argument merges over `source.poster`, so you can vary one modifier without rebuilding the source — `createMuxPosterURL(video.source, { width: 320 })`, for instance.
