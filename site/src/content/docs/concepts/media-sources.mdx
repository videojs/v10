...
To vary a modifier without touching `source` — asking for a narrower image on a small viewport, say — build the URL directly. The second argument merges over `source.poster`:

```ts
import { createMuxPosterURL } from '@videojs/media/dom/mux';

const small = createMuxPosterURL(video.source, { width: 320 });
```
