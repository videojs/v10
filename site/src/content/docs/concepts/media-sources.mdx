...
It's read-only and tracks `source`, so read it again after `sourcechange`. It's empty when there is no playback ID, and when signed playback has no matching image token.
