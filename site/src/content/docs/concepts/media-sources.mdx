...
Three separate mechanisms bound which rendition plays, and they're easy to mistake for each other:
...
`playback.maxResolution` is the server-side one: it decides what the manifest carries. The other two only bound what adaptive selection picks from whatever does arrive.
