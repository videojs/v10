...
For example, the `/background` preset includes a media element with autoplay, mute, and loop built in, a skin with no controls, and no features — because there's no UI that needs state:
...
<background-video-player>
  <background-video-skin>
    <video slot="media" src="hero.mp4" autoplay muted loop playsinline></video>
  </background-video-skin>
</background-video-player>
```
</FrameworkCase>
...
- **Feature bundle** — An array of <DocsLink slug="concepts/features">features</DocsLink> that define the player's state and actions. The video feature bundle includes playback, volume, time, fullscreen, and more. The background feature bundle includes none — there's no UI that needs state.
- **Skins** — Pre-built UIs designed for that feature bundle. A video skin renders fullscreen and PIP controls because the video feature bundle includes those features. An audio skin doesn't.
- **Media element** — A media component suited to the use case. The background video element bakes in autoplay, mute, and loop. The default video and audio elements are thin wrappers around their native HTML counterparts.

These parts aren't equally coupled. Skins depend tightly on their feature bundle — a skin expects specific features to exist. Media elements are more interchangeable — you can swap in an HLS or DASH provider without changing your skin or features.
