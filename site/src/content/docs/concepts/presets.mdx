...
Registration is explicit, so importing a value never changes the custom element registry:

```ts
import '@videojs/html/video/player'; // <video-player> only
import '@videojs/html/video/ui';     // <media-container> + video UI
import '@videojs/html/video/skin';   // <video-skin> + container + video UI
```

A complete default player imports `video/player` and `video/skin`. Import `video/ui` instead of the skin when rendering an ejected light-DOM layout.
</FrameworkCase>
