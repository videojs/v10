...
<FrameworkCase frameworks={["react"]}>
Each React preset exports a named, preconfigured player component and typed `usePlayer` hook, plus the building blocks needed to customize it or own its source:
...
You can customize a preset in three ways: extend its feature bundle, own its skin's source, or swap its media element.
...
### Own the skin's source

If a preset's skin is close but not quite right, <DocsLink slug="how-to/customize-skins" anchor="own-and-customize-the-source">install an editable copy</DocsLink> and customize its components and styles.
