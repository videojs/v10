...
In HTML, always add `slot="media"` to your media element. Skins contain a <DocsLink slug="reference/player-container">`<media-container>`</DocsLink> internally that uses this slot to discover the media.
</FrameworkCase>
...
Skins are designed for <DocsLink slug="concepts/presets">presets</DocsLink>. A preset is a starting set of <DocsLink slug="concepts/features">features</DocsLink> grouped together for a specific use case. The default presets are simply `video` and `audio`. These cover a baseline set of features, roughly the set of controls you'd expect from the HTML `<video>` and `<audio>` tags.
...
<DocsLinkCard slug="concepts/presets">Read more about presets</DocsLinkCard>
