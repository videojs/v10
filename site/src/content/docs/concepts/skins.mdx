...
When you choose a skin you have two options for how you use it: **packaged** or **open**. It's usually easiest to start with a packaged skin and later copy its internal components into your project when you need more customization.
...
## Skins and Player Features

Skins are designed for Presets [LINK ME]. A Preset is a starting set of features grouped together for a specific use case. The default presets are simply `video` and `audio`. These cover a baseline set of features, roughly the set of controls you'd expect from the HTML `<video>` and `<audio>` tags.

Beyond the defaults, presets target more specific use cases. The `background-video` preset, for example, needs layout but not controls. Over time we'll add more, for example creator-platform players, short-form swipe-up players, TV app streaming players. On the audio side, a podcast player and a music player have a different set of needs. Presets give these use cases their own tailored feature sets.

Because skins are tied to presets, each skin is built for the specific features that preset includes. The default video and audio presets ship with multiple skin options, and those options will grow over time. Non-default presets will typically have a single skin, since the use case is more focused.

With any skin, you can use it as a starting point and eject (or use the open version) to build on top of it.

<FrameworkCase frameworks={["html"]}>
| Player Preset | Available Skins |
|----------|-----------------|
| `<video-player>` | `<video-skin>`, `<minimal-video-skin>` |
| `<audio-player>` | `<audio-skin>`, `<minimal-audio-skin>` |
...
| Player Preset | Available Skins |
|----------|-----------------|
| `videoFeatures` | `<VideoSkin>`, `<MinimalSkin>` |
| `audioFeatures` | `<AudioSkin>`, `<MinimalAudioSkin>` |
| `backgroundVideoFeatures` | `<BackgroundVideoSkin>` |
</FrameworkCase>
