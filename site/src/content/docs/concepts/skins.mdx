...
Each skin is built for a specific <DocsLink slug="concepts/features">feature bundle</DocsLink> — it expects certain features to be available. A video skin renders fullscreen and picture-in-picture controls because the video feature bundle includes those features. An audio skin doesn't.

A <DocsLink slug="concepts/presets">preset</DocsLink> packages a feature bundle together with its compatible skins and a media element. The default video and audio presets ship with multiple skin options, and those options will grow over time. More focused presets will typically have a single skin.

<FrameworkCase frameworks={["html"]}>

| Preset | Available skins |
|--------|-----------------|
| `/video` | `<video-skin>`, `<video-minimal-skin>` |
| `/audio` | `<audio-skin>`, `<audio-minimal-skin>` |
| `/background` | `<background-video-skin>` |
...
| Preset | Available skins |
|--------|-----------------|
| `/video` | `<VideoSkin>`, `<MinimalVideoSkin>` |
| `/audio` | `<AudioSkin>`, `<MinimalAudioSkin>` |
| `/background` | `<BackgroundVideoSkin>` |
