...
Skins are designed for specific presets. A preset is a curated set of features bundled for a specific use case. 

<DocsLinkCard slug="concepts/presets">Read more about presets</DocsLinkCard>

Because skins are tied to presets, each skin is built for the specific features that preset includes. The default video and audio presets ship with multiple skin options, and those options will grow over time. Other presets will typically have a single skin, since the use case is more focused.

<FrameworkCase frameworks={["html"]}>
| Player preset | Available skins | Import |
|----------|-----------------|--------|
...
For example, combining the video preset features and skin would look like this:
