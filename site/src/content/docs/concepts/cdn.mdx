...
A skin bundle registers every UI element its skin is built from. The CDN publishes no per-component bundles, though. If you have <DocsLink slug="how-to/change-a-skins-controls-and-layout">installed an editable copy of a skin</DocsLink> and need to add new components, install from npm instead of from the CDN.
