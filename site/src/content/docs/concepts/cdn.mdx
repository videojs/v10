...
A skin bundle registers every UI element its skin is built from. The CDN publishes no per-component bundles, though. If you've <DocsLink slug="how-to/customize-skins">installed the skin's source</DocsLink> and need to add new components, install from npm instead of from the CDN.
