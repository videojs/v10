...
<DocsLink slug="concepts/extensions">Extensions</DocsLink> sit alongside state, UI, and media. Add one when a player needs optional behavior such as Google Cast or Mux Data monitoring; it uses the active media without rendering UI or playing media itself.

## Presets

**Presets** preconfigure the state, UI, and media for a specific use case. Add extensions when that player also needs an optional integration.
