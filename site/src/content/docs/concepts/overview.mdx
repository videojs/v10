...
**Presets** package these parts together for a specific use case. They can include special state management, one or more skins for UI, and specific media elements. Instead of wiring these up individually, you pick a preset that matches what you're building.

The default presets are `/video` and `/audio`, covering the baseline set of controls you'd expect from the HTML `<video>` and `<audio>` tags. Beyond the defaults, presets target more specific use cases. For example, `/background`:
