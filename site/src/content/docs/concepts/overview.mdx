...
You can access state and actions from anywhere within `VideoPlayer` with the <DocsLink slug="components/use-player">`usePlayer` hook</DocsLink>.
...
You can access state and actions from anywhere within `<video-player>` with <DocsLink slug="components/player-controller">`PlayerController`</DocsLink>.
</FrameworkCase>
...
An extension can provide behavior that a UI component controls: GoogleCast supplies the configurable Cast route, and <DocsLink slug="components/cast-button">CastButton</DocsLink> renders the control that starts and ends a session.
