...
<EngineOptions media="HlsJsVideo" engine="hlsJs" />

### Native playback

When playback falls back to the browser's own HLS, hls.js isn't involved and its options don't apply. `source.engine.nativeHls` configures that path instead.

<EngineOptions media="HlsJsVideo" engine="nativeHls" />
