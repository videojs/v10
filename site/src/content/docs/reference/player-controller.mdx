...
The controller returned by <DocsLink slug="reference/html-create-player">`createPlayer`</DocsLink> is already bound to that player's context, so pass only the host and optional selector. The lower-level `PlayerController` exported from `@videojs/html` accepts the context as its second argument; its constructor signatures are documented below.
