...
`Thumbnail.Root` resolves an image for the current `time`. `Thumbnail.Img` renders the selected image and reports its loading lifecycle to the root.
...
```tsx
<Thumbnail.Root time={12}>
  <Thumbnail.Img crossOrigin={null} />
</Thumbnail.Root>
```
</FrameworkCase>
...
<FrameworkCase frameworks={["react"]}>
React renders a root `<div>` around an `<img>`. State attributes belong to `Thumbnail.Root`, while image attributes and `render` belong to `Thumbnail.Img`:

```tsx
<Thumbnail.Root className="media-thumbnail" time={12}>
  <Thumbnail.Img className="media-thumbnail-img" loading="eager" />
