...
<FrameworkCase frameworks={["react"]}>
`CaptionsRadioGroup.Root` owns selection and publishes option state to an enclosing Menu. `Value` displays the selected label. `Options` generates each item, including `Off`, and returns `null` when the <DocsLink slug="reference/feature-text-tracks">text tracks feature</DocsLink> is not configured.
</FrameworkCase>
