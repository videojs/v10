...
├── BasicUsage.astro    # Wrapper: renders HTML and bundles script
...
### CSS naming and scoping

Use component-mapped class names (no BEM). Keep demo CSS source clean; demo scoping is handled automatically by the shared `Demo.astro` shell.
...
- Inner classes should map to component/tag names where possible (`media-container`, `media-play-button`, `media-time-slider`)
- For generic wrappers, use short semantic names (`controls`, `panel`, `toggle`)
- React and HTML demos for the same variant should use the same class names
