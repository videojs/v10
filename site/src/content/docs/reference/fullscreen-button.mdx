...
Renders a `<button>` with an automatic `aria-label`: "Enter fullscreen" or "Exit fullscreen". Override with the `label` prop. Keyboard activation: <kbd>Enter</kbd> / <kbd>Space</kbd>. Both disabled and unavailable states set `aria-disabled="true"` so screen readers announce the button as non-interactive.
