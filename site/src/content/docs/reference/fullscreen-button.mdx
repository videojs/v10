...
<FrameworkCase frameworks={["html"]}>
The default skins automatically hide the button when fullscreen is unsupported. For custom skins, use the `data-available` attribute:

```css
media-fullscreen-button:not([data-available]) {
  display: none;
}
```
</FrameworkCase>
