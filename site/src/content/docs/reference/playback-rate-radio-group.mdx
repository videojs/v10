...
<FrameworkCase frameworks={["react"]}>
`PlaybackRateRadioGroup.Root` owns selection and publishes option state to an enclosing Menu. `Value` displays the selected label. `Options` generates each item and returns `null` when the <DocsLink slug="reference/feature-playback-rate">playback rate feature</DocsLink> is not configured.
</FrameworkCase>
