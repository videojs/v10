...
`title` already means the tooltip on an element, so the player takes the title as
`content-title`, reflected by the `contentTitle` property. Set it through a template ref:
  ...
  onMounted(() => {
    player.value!.contentTitle = "Sintel";
  });
  </script>
...
`title` already means the tooltip on an element, so the player takes the title as
`content-title`, reflected by the `contentTitle` property. Set it through a bound element:
