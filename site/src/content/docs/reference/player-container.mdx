...
If you omit `VideoSkin` to build custom UI, render `Container` yourself and supply its size and positioning styles:
...
If you omit `<video-skin>` to build custom UI, render `<media-container>` yourself and supply its display, size, and positioning styles:

```html
<!-- Packaged skin — container is inside video-skin -->
<video-player>
  <video-skin class="player-surface">
    <video src="video.mp4"></video>
  </video-skin>
</video-player>
