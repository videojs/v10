...
<FrameworkCase frameworks={["html"]}>
Custom video elements such as `<mux-video>` and `<hlsjs-video>` are also boxless: their inner `<video>` fills the parent. Size and measure the container, not the custom media host. Native `<video>` and `<audio>` elements still render their own boxes.
</FrameworkCase>
<FrameworkCase frameworks={["react"]}>
...
Style the skin for the player's outer size and aspect ratio. The packaged skin CSS makes its container fill that space and supplies the positioning rules for media, controls, posters, and overlays. React loads those rules from the skin CSS import; importing an HTML skin registers the styles with the skin element.

Headless player and media imports do not include skin layout. Render `Container` or `<media-container>` yourself and supply its display, size, and positioning styles.
