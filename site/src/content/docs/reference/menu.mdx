...
A root menu owns one positioned popup. Each `Content` is a menu page; nested pages share that popup with the root page.
...
<FrameworkCase frameworks={["html"]}>
  ...
  <media-menu id="settings-menu">
    ...
    <!-- Sibling page opened by the parent item -->
...
In React, create a submenu by nesting another `Menu.Root` in the parent `Menu.Content`; its `Menu.Trigger` behaves as a parent item and its `Menu.Content` becomes the next page. In HTML, add another `<media-menu-content>` as a sibling page, give it an `id`, and point the parent `<media-menu-item>` at it with `commandfor`.
...
The root `Menu.Popup` or `<media-menu>` receives `data-open`, `data-side`, and `data-align`. Each Content page receives menu state such as `data-open` and `data-submenu`; a parent page receives `data-submenu-expanded` while one of its child pages is opening or closing.
...
<FrameworkCase frameworks={["react"]}>
  ...
  .menu-content[data-submenu-expanded="true"] {
    translate: -100% 0;
  }
...
<FrameworkCase frameworks={["html"]}>
  ...
  media-menu-content[data-submenu-expanded="true"] {
    translate: -100% 0;
  }
