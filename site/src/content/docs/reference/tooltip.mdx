...
<FrameworkCase frameworks={["html"]}>
  ...
  discovers its trigger automatically and manages open/close state and positioning. Wrap tooltip trigger/popup pairs in `<media-tooltip-group>` to coordinate
...
Tooltips are **visual-only** — they do not set `aria-describedby` on triggers or use `role="tooltip"`. The popup renders with `role="presentation"` and `popover="manual"`, so screen readers skip it entirely. Accessible names for media buttons are provided directly via `aria-label` on the button itself, not through the tooltip.

When a tooltip wraps a media button, the button's resolved label is automatically forwarded as the tooltip's text content. This keeps the visual tooltip in sync with the button's accessible name without duplicating it in the accessibility tree. Tooltips open on focus and close when focus leaves, ensuring keyboard-only users can see the label visually.
