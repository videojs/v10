...
<FrameworkCase frameworks={["react"]}>
`AudioTrackRadioGroup.Root` owns selection and publishes option state to an enclosing Menu. `Value` displays the selected label. `Options` generates each item through its required `renderItem` callback and returns `null` when the <DocsLink slug="reference/feature-audio-track">audio track feature</DocsLink> is not configured.
</FrameworkCase>
