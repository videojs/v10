...
Both inputs are props on <DocsLink slug="reference/player-provider">`Player.Provider`</DocsLink>. A player built without the metadata feature has neither prop, and neither is forwarded.
