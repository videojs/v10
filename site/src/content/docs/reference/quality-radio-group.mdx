...
<FrameworkCase frameworks={["react"]}>
<DocsLink slug="reference/use-video-quality-options">`useVideoQualityOptions`</DocsLink> returns `null` when the <DocsLink slug="reference/feature-quality">quality feature</DocsLink> is not configured. Use the returned options with <DocsLink slug="reference/menu">`Menu.RadioGroup`</DocsLink>.
</FrameworkCase>
