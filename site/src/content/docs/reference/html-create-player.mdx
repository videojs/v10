...
```ts title="player.ts"
import { ContainerMixin, createPlayer, UIElement, selectPlayback } from '@videojs/html';
...
class PlayerRegion extends ContainerMixin(UIElement) {}
```
...
class ComposedPlayer extends ProviderMixin(ContainerMixin(UIElement)) {}
```
