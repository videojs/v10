...
`PlayerElement` and `PlayerController` are scoped to the created player's feature set. The element owns its store and attachment lifecycle; the configured controller does not need a context argument.
