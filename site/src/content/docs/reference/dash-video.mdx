...
Pass dash.js's own settings under `source.engine.dashJs`. They're handed to dash.js untouched, so every option in its [settings reference](https://cdn.dashjs.org/latest/jsdoc/module-Settings.html) works here. Replacing the object resets any settings applied before it. <DocsLink slug="concepts/media-sources">Media Sources</DocsLink> covers how engine options fit into a structured source.

<EngineOptions media="DashVideo" engine="dashJs" />

Each of those is a group of dash.js settings rather than a single value, and dash.js documents what's inside them.
