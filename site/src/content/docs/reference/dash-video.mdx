...
{/* React demos */}
import BasicUsageDemoReact from "@/components/docs/demos/dash-video/react/css/BasicUsageClient";
...
{/* Unlike every other media element, DashVideo can't be server-rendered: dashjs bundles
    imsc, which reads `window.getComputedStyle` at module scope (#1343). BasicUsageClient
    defers the import to the browser, and the wrapper holds the demo's 16/9 box so
  ...
  <StyleCase styles={["css"]}>
    <Demo files={[
      ...
      <div class="aspect-video w-full">
        <BasicUsageDemoReact client:only="react" />
      </div>
    </Demo>
  </StyleCase>
</FrameworkCase>
