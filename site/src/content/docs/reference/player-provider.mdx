...
A headless import does not load the skin styles that set `display: contents`. The provider still owns state, not layout, so add and style a container instead of relying on the provider's default display.
</FrameworkCase>
