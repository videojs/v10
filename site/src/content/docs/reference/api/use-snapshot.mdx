...
| <DocsLink slug="reference/api/use-store">`useStore`</DocsLink> | Store instance | Store-backed state with features |

Use `useSnapshot` when working with standalone `State` containers outside the player store system -- for example, custom state in component libraries. For player state, use <DocsLink slug="reference/api/use-player">`usePlayer`</DocsLink> or <DocsLink slug="reference/api/use-store">`useStore`</DocsLink>.

`useSnapshot` is built on <DocsLink slug="reference/api/use-selector">`useSelector`</DocsLink> and uses `shallowEqual` by default. The HTML equivalent is `SnapshotController`.
