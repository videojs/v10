...
|                | `SnapshotController`           | <DocsLink slug="reference/api/store-controller">`StoreController`</DocsLink> |
| -------------- | ------------------------------ | ------------------------------------------------------------------------ |
...
<DocsLink slug="reference/api/store-controller">`StoreController`</DocsLink> is
...
`SnapshotController` subscribes on `hostConnected()` and unsubscribes on `hostDisconnected()`. The React equivalent is <DocsLink slug="reference/api/use-snapshot">`useSnapshot`</DocsLink>.
