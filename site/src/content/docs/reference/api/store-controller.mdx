...
| | `StoreController` | <DocsLink slug="reference/api/player-controller">`PlayerController`</DocsLink> | <DocsLink slug="reference/api/snapshot-controller">`SnapshotController`</DocsLink> |
|---|---|---|---|
...
<DocsLink slug="reference/api/player-controller">`PlayerController`</DocsLink> is a typed wrapper around `StoreController` scoped to the player store. For player UI, prefer `PlayerController`. Use `StoreController` when working with a custom store outside the player system.

When you pass a selector, `StoreController` internally creates a <DocsLink slug="reference/api/snapshot-controller">`SnapshotController`</DocsLink> on the store's `$state` container. Without a selector, no subscription is created -- you get the store instance for imperative access to actions.
