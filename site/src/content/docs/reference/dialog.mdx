...
<FrameworkCase frameworks={["html"]}>
Place a button immediately before `media-dialog`, or connect them explicitly with `commandfor` and an `id`. You can also set the dialog's `open` property directly. The element dispatches `open-change` when its state changes and `open-change-complete` after its transition finishes.
</FrameworkCase>
...
<FrameworkCase frameworks={["html"]}>
```css
media-dialog:not([data-open]) {
  display: none;
}

media-dialog[data-starting-style],
media-dialog[data-ending-style] {
  opacity: 0;
}
```
</FrameworkCase>
