...
frameworkTitle:
  html: spotify-audio
description: Audio element that plays Spotify tracks, episodes, playlists, and shows through the Spotify embed
---
...
Audio element that plays Spotify content through the [Spotify embed](https://developer.spotify.com/documentation/embeds): tracks, episodes, albums, playlists, shows, and artists. Podcast episodes play in full for signed-out listeners; music plays as a preview until the listener logs in to Spotify.
...
The Spotify embed exposes less than the other providers':
...
- `loop` replays the entity from the top when it ends; without it, the embed rolls on into whatever follows.

## Embed options

Pass [Spotify embed options](https://developer.spotify.com/documentation/embeds) under `source.engine.spotify`, spelled exactly as Spotify spells them. <DocsLink slug="concepts/media-sources">Media Sources</DocsLink> covers how engine options fit into a structured source.

| Option | It sets |
|---|---|
| `t` | Start position in seconds; outranks the `t` parameter on `src` |
| `theme` | `0` for the dark theme; omit for the default |
| `preferVideo` | `true` to load the video variant of an episode that has one |

Anything Spotify adds next passes through the same way.
