...
<FrameworkCase frameworks={["react"]}>
`StatusAnnouncer` watches player-store snapshots, so it reports changes made by buttons, sliders, custom controls, or direct player actions. It does not depend on <DocsLink slug="reference/components/hotkey">`Hotkey`</DocsLink> or <DocsLink slug="reference/components/gesture">`Gesture`</DocsLink> events.
</FrameworkCase>

<FrameworkCase frameworks={["html"]}>
`<media-status-announcer>` watches player-store snapshots, so it reports changes made by buttons, sliders, custom controls, or direct player actions. It does not depend on <DocsLink slug="reference/components/hotkey">`<media-hotkey>`</DocsLink> or <DocsLink slug="reference/components/gesture">`<media-gesture>`</DocsLink> events.
</FrameworkCase>
...
<FrameworkCase frameworks={["react"]}>
Use <DocsLink slug="reference/components/seek-indicator">`SeekIndicator`</DocsLink> for temporary visual seek feedback. Keep that visual value out of a live region; `StatusAnnouncer` owns the accessible announcement.
</FrameworkCase>

<FrameworkCase frameworks={["html"]}>
Use <DocsLink slug="reference/components/seek-indicator">`<media-seek-indicator>`</DocsLink> for temporary visual seek feedback. Keep that visual value out of a live region; `<media-status-announcer>` owns the accessible announcement.
</FrameworkCase>
