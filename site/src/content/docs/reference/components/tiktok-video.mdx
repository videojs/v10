...
<MediaImports media="tiktok-video" package="@videojs/tiktok-video" react="TikTokVideo" />
...
TikTok always draws part of its own chrome: the center play button, author header, and social rail are always visible. `controls` adds the progress bar and control buttons on top of those. Without `controls`, the element ignores pointer events, so a <DocsLink slug="guides/skins">player skin</DocsLink> above it receives every click.
...
<MediaReference media="TikTokVideo">
  <Fragment slot="engine-tiktok">
    Pass [TikTok player parameters](https://developers.tiktok.com/doc/embed-player) under `source.engine.tiktok`, spelled exactly as TikTok spells them, each one `0` or `1`, plus anything TikTok adds next. <DocsLink slug="guides/media-sources">Media Sources</DocsLink> covers how engine options fit into a structured source.
