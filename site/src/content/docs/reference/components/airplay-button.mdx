...
<FrameworkCase frameworks={["react"]}>
The component consumes the unified <DocsLink slug="reference/api/feature-remote-playback">`remotePlayback`</DocsLink> store feature alongside `CastButton`: both buttons drive their state from the same feature, but each only surfaces on its supported platform (WebKit for AirPlay, Chromium for Cast).
</FrameworkCase>

<FrameworkCase frameworks={["html"]}>
The component consumes the unified <DocsLink slug="reference/api/feature-remote-playback">`remotePlayback`</DocsLink> store feature alongside `<media-cast-button>`: both buttons drive their state from the same feature, but each only surfaces on its supported platform (WebKit for AirPlay, Chromium for Cast).
</FrameworkCase>
