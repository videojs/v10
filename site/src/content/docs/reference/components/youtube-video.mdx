...
The [YouTube IFrame player](https://developers.google.com/youtube/iframe_api_reference) keeps its own chrome hidden by default, so it drops into a <DocsLink slug="guides/skins">player skin</DocsLink> like any other media component; set `controls` to use YouTube's UI instead.

## Import

<MediaImports media="youtube-video" package="@videojs/youtube-video" react="YouTubeVideo" />
