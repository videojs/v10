...
HLS video element that relies on the browser's native HLS support. Works on Safari and other browsers with built-in HLS playback. For cross-browser HLS support, use the <DocsLink slug="reference/components/hlsjs-video">hls.js video component</DocsLink> instead.

## Import

<MediaImports media="native-hls-video" react="NativeHlsVideo" />
...
<MediaReference media="NativeHlsVideo">
  <Fragment slot="engine-nativeHls">
    This element hands playback to the browser, so there's no JavaScript engine to configure. This carries the one thing native playback still needs from you. <DocsLink slug="guides/media-sources">Media Sources</DocsLink> covers how engine options fit into a structured source.
