...
Audio-only HLS element that plays just the audio rendition of an HLS stream — even when the source is a mixed audio/video manifest. Useful for audio podcast players, background audio, and bandwidth-constrained contexts where downloading video data would be wasted. For full audio/video HLS playback, see <DocsLink slug="reference/components/hls-video">HlsVideo</DocsLink>.

## Import

<MediaImports media="hls-audio" react="HlsAudio" />
