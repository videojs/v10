...
The component shows the name of what is playing. The root owns visibility and the value part renders the resolved text. Set the
title on the player; compose other content alongside the value part.
...
That resolution belongs to the <DocsLink slug="reference/api/feature-metadata">metadata
...
The root exposes `data-visible` while controls are visible. Use that hook to animate the
title with the controls; the value part owns only the title text.
...
```tsx
<Title.Root className={(state) => (state.title.length > 40 ? "title title--long" : "title")}><Title.Value /></Title.Root>
```
</FrameworkCase>
