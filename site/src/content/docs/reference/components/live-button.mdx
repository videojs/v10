...
The button combines three player features: <DocsLink slug="reference/api/feature-live">live</DocsLink> for `liveEdgeStart` and `targetLiveWindow`, <DocsLink slug="reference/api/feature-time">time</DocsLink> for `currentTime` and the `seek()` action, and <DocsLink slug="reference/api/feature-buffer">buffer</DocsLink> for `seekable`. All three are included in the `liveVideoFeatures` and `liveAudioFeatures` presets, and the packaged live skins include the button.
...
See <DocsLink slug="guides/play-live-streams">Play live streams</DocsLink> for how live state flows through the player.
