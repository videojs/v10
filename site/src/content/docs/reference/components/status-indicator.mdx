...
<ComponentImports component="StatusIndicator" html={["status-indicator", "status-indicator-value"]} />
...
<FrameworkCase frameworks={["react"]}>
`StatusIndicator` displays feedback for actions emitted by a <DocsLink slug="reference/components/hotkey">`Hotkey`</DocsLink> or <DocsLink slug="reference/components/gesture">`Gesture`</DocsLink> in the same player `Container`. It does not react to buttons, direct player-store changes, or media state changes on their own.
</FrameworkCase>

<FrameworkCase frameworks={["html"]}>
`<media-status-indicator>` displays feedback for actions emitted by a <DocsLink slug="reference/components/hotkey">`<media-hotkey>`</DocsLink> or <DocsLink slug="reference/components/gesture">`<media-gesture>`</DocsLink> in the same player `<media-container>`. It does not react to buttons, direct player-store changes, or media state changes on their own.
</FrameworkCase>
...
<FrameworkCase frameworks={["react"]}>
`StatusIndicator` is visual feedback and does not create a live region. Keep every action available through keyboard-operable controls, and pair the player with <DocsLink slug="reference/components/status-announcer">`StatusAnnouncer`</DocsLink> when state changes should be announced to screen readers. Do not make `StatusIndicator.Value` a live region.
</FrameworkCase>

<FrameworkCase frameworks={["html"]}>
`<media-status-indicator>` is visual feedback and does not create a live region. Keep every action available through keyboard-operable controls, and pair the player with <DocsLink slug="reference/components/status-announcer">`<media-status-announcer>`</DocsLink> when state changes should be announced to screen readers. Do not make `<media-status-indicator-value>` a live region.
</FrameworkCase>
