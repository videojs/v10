...
The storyboard lives on a Mux domain, so MuxVideo has to be CORS-enabled for that cross-origin `<track>` to load at all. <DocsLink slug="reference/components/thumbnail">Thumbnail</DocsLink> then fetches the sprite sheets its cues point at the same way:
...
<DocsLinkCard slug="guides/monitor-playback-with-mux-data" description="Metadata, options, and configuration">Mux Data</DocsLinkCard>
<DocsLinkCard slug="guides/cast-to-airplay-and-chromecast" description="Receivers, load requests, and session state">Cast to AirPlay and Chromecast</DocsLinkCard>
