...
<ComponentImports component="VolumeIndicator" html={["volume-indicator", "volume-indicator-fill", "volume-indicator-value"]} />
...
<FrameworkCase frameworks={["react"]}>
`VolumeIndicator` displays feedback for `toggleMuted` and `volumeStep` actions emitted by a <DocsLink slug="reference/components/hotkey">`Hotkey`</DocsLink> or <DocsLink slug="reference/components/gesture">`Gesture`</DocsLink> in the same player `Container`. It does not react to mute buttons, volume sliders, direct player-store changes, or volume changes on their own.
</FrameworkCase>

<FrameworkCase frameworks={["html"]}>
`<media-volume-indicator>` displays feedback for `toggleMuted` and `volumeStep` actions emitted by a <DocsLink slug="reference/components/hotkey">`<media-hotkey>`</DocsLink> or <DocsLink slug="reference/components/gesture">`<media-gesture>`</DocsLink> in the same player `<media-container>`. It does not react to mute buttons, volume sliders, direct player-store changes, or volume changes on their own.
</FrameworkCase>
...
<FrameworkCase frameworks={["react"]}>
`VolumeIndicator` is visual feedback and does not create a live region. Keep volume and mute available through keyboard-operable controls, and pair the player with <DocsLink slug="reference/components/status-announcer">`StatusAnnouncer`</DocsLink> when state changes should be announced to screen readers. Do not make `VolumeIndicator.Value` a live region.
</FrameworkCase>

<FrameworkCase frameworks={["html"]}>
`<media-volume-indicator>` is visual feedback and does not create a live region. Keep volume and mute available through keyboard-operable controls, and pair the player with <DocsLink slug="reference/components/status-announcer">`<media-status-announcer>`</DocsLink> when state changes should be announced to screen readers. Do not make `<media-volume-indicator-value>` a live region.
</FrameworkCase>
