...
[Cloudflare Stream](https://developers.cloudflare.com/stream/) runs in an embedded player. Its own chrome stays hidden by default, so it drops into a <DocsLink slug="guides/skins">player skin</DocsLink> like any other media component; set `controls` to use Cloudflare's UI instead.

## Import

<MediaImports media="cloudflare-video" package="@videojs/cloudflare-video" react="CloudflareVideo" />
