...
<ComponentImports component="SeekIndicator" html={["seek-indicator", "seek-indicator-value"]} />
...
<FrameworkCase frameworks={["react"]}>
`SeekIndicator` displays feedback for seek actions emitted by a <DocsLink slug="reference/components/hotkey">`Hotkey`</DocsLink> or <DocsLink slug="reference/components/gesture">`Gesture`</DocsLink> in the same player `Container`. It does not react to current-time changes or seek commands invoked directly through the player store.
</FrameworkCase>

<FrameworkCase frameworks={["html"]}>
`<media-seek-indicator>` displays feedback for seek actions emitted by a <DocsLink slug="reference/components/hotkey">`<media-hotkey>`</DocsLink> or <DocsLink slug="reference/components/gesture">`<media-gesture>`</DocsLink> in the same player `<media-container>`. It does not react to current-time changes or seek commands invoked directly through the player store.
</FrameworkCase>
...
<FrameworkCase frameworks={["react"]}>
`SeekIndicator` is visual feedback and does not create a live region. Keep the same seek operation available through keyboard-operable controls, and pair it with <DocsLink slug="reference/components/status-announcer">`StatusAnnouncer`</DocsLink> when changes should be announced to screen readers. Do not make `SeekIndicator.Value` a live region because rapid seek input would produce repeated announcements.
</FrameworkCase>

<FrameworkCase frameworks={["html"]}>
`<media-seek-indicator>` is visual feedback and does not create a live region. Keep the same seek operation available through keyboard-operable controls, and pair it with <DocsLink slug="reference/components/status-announcer">`<media-status-announcer>`</DocsLink> when changes should be announced to screen readers. Do not make `<media-seek-indicator-value>` a live region because rapid seek input would produce repeated announcements.
</FrameworkCase>
