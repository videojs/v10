...
The base slider provides a generic range input. It manages value, pointer tracking, and drag interactions. Domain-specific sliders like <DocsLink slug="reference/components/time-slider">TimeSlider</DocsLink> and <DocsLink slug="reference/components/volume-slider">VolumeSlider</DocsLink> extend this with media-specific bindings. The time slider also provides parts for rendering chapter ranges and the chapter at the current interaction position.
