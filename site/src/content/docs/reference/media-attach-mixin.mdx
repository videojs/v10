...
`MediaAttachMixin` creates a class that registers itself with the <DocsLink slug="reference/player-provider">player</DocsLink> on connect. Apply it to custom media elements so they automatically wire to the store.
