...
The base Slider is a **controlled component** — you must provide `value` and `onValueChange` to drive it. It manages pointer tracking and drag interactions but does not hold internal state. Domain-specific sliders like <DocsLink slug="reference/time-slider">TimeSlider</DocsLink> and <DocsLink slug="reference/volume-slider">VolumeSlider</DocsLink> handle this automatically via the player store.
