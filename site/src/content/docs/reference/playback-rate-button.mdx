...
Renders a `<button>` with `role="spinbutton"`, `aria-label` `"Playback speed"` (override with the `label` prop), `aria-valuenow` / `aria-valuetext` (e.g. `1×`), and `aria-valuemin` / `aria-valuemax` from the current `playbackRates` list when it is non-empty. Tooltip copy matches the `aria-label`. Activate with <kbd>Enter</kbd> / <kbd>Space</kbd> or the speed-up hotkey.
