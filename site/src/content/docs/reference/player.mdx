...
frameworkTitle:
  html: video-player
description: The state boundary — creates a store and makes it available to all descendants.
---
...
<FrameworkCase frameworks={["html"]}>
Use `PlayerController` to subscribe to store state from any custom element inside the player:

```ts title="play-pause-button.ts"
import { MediaElement, PlayerController, playerContext, selectPlayback } from '@videojs/html';
