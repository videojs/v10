...
This element hands playback to the browser, so there's no JavaScript engine to configure. `source.engine.nativeHls` carries the one thing native playback still needs from you. <DocsLink slug="concepts/media-sources">Media Sources</DocsLink> covers how engine options fit into a structured source.

<EngineOptions media="NativeHlsVideo" engine="nativeHls" />
