...
Media counts as live while `targetLiveWindow` is not `NaN` (`0` for a sliding live window, `Infinity` for DVR). For on-demand media the button reports neither `live` nor `liveEdge` and stays inert. The live edge begins 5 seconds before `liveEdgeStart`; when the media does not report `liveEdgeStart`, the edge is the trailing 10 seconds of the seekable window instead.
