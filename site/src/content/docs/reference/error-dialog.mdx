...
`ErrorDialog` builds on <DocsLink slug="reference/dialog">`Dialog`</DocsLink> with alert semantics and is driven by the player's <DocsLink slug="reference/feature-error">error feature</DocsLink>. It re-exports the dialog backdrop and popup under the `ErrorDialog` namespace. It opens when the attached media reports an error and stays hidden otherwise. Closing it dismisses the current error in the player store.
