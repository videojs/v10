...
**Changelog prose and blog posts** arrive between releases. The prose bot only starts once the release is published, so its PR lands on `main` after production has already moved, and blog posts merge whenever they are ready. The [Forward-port changelog and blog](../.github/workflows/forward-port-changelog.yml) workflow closes the gap: whenever anything under `src/content/changelog/`, `src/content/blog/`, or `src/assets/blog/` changes on `main`, it copies those folders onto `site/v10`. No cherry-pick needed. A blog post that merges before it should be public needs `devOnly: true` in its frontmatter, and a post that imports a component new to `main` needs the cherry-pick route below instead, since only those folders are copied.
