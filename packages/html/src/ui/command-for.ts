...
export function toggleCommandTarget(host: HTMLElement, commandfor: string): void {
  const root =
    /* SAFETY: The surrounding typed API establishes the asserted contract at this boundary. */ host.getRootNode() as
      | Document
      | ShadowRoot;
  const target =
  ...
  const popup = /* SAFETY: The surrounding typed API establishes the asserted contract at this boundary. */ target as {
    open: boolean;
  };
