...
/**
 * `stop` — abort the pending queue and return to `idle`. The in-flight
 * fetch is left to complete. Sent by the suspend path in `load-segments.ts`
 * while `remotePlaybackActive`.
 */
