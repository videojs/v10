...
describe('createSegmentLoaderActor — stop message', () => {
  it('drops the queued batch and returns to idle', async () => {
    ...
    // fetch is left to complete — the SPF `stopLoad()` analogue deliberately
    // doesn't poke the SourceBuffer / MediaSource, which can disrupt an
    // in-flight AirPlay handoff.)
