...
function commonTemplate(tag: string): MediaTemplate {
  return (attrs) => /*html*/ `
    <style>
      :host {
        display: inline-flex;
...
/** The bare template for a host: a `<slot name="media">` wrapping the host element, and a default slot after it. */
export function defaultTemplate(host: AdapterHost): MediaTemplate {
  return host === 'video' ? videoTemplate : commonTemplate(host);
}
