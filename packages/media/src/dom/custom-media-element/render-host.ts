...
export interface InitialMediaAttributes {
 ...
 */
export function initialAttributes(element: HTMLElement, routes: AttributeRoutes): InitialMediaAttributes {
  const all = pick(namedNodeMapToObject(element.attributes), routes.observed);

  return { all, target: omit(all, [...routes.ownerOf.keys()]) };
}
