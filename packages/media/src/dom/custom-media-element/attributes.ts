 ...
 */
export const mediaAttributeConfigs: Readonly<Record<string, Pick<AttributeConfig, 'attribute' | 'state'>>> = {
...
/**
 * @param native - The host element's own attributes, owned by the adapter only where it can set the property.
...
export function buildRoutes(
  native: AttributeConfigs,
  derived: AttributeConfigs,
  ...
  const configs: Record<string, { config: AttributeConfig; owned: boolean }> = {};

  for (const [prop, config] of Object.entries(native)) {
