 ...
 * evaluation registers the element and the only thing React takes from it, so naming it somewhere that pulls
 * none of Wistia in would leave a bundler free to drop this module and React rendering an undefined tag.
 */
...
/**
 * Wistia is the one media here that ships a web component of its own, so this is that component rather than a
 * wrapper around one; the platform packages give it a tag and a player store.
 *
 * `normalizeWistiaPlayer` gives the player the members and events `HTMLMediaElement` has that Wistia names
 * differently or not at all. This class does the rest: the attributes a media element is written with, which
 * Wistia has its own names, spellings, and defaults for.
 */
 ...
 *
 * The five that configure the player go through `applyOptions` together, which is what lets `controls` going
 * away put the player back the way this media would have had it rather than the way Wistia would. Wistia
 * observes `autoplay`, `poster`, and `preload` too, and these readings win. The other three mean something
 * on their own: Wistia has no `src` and names its media by id, `muted` is the state to start in rather than
 * the live one a mute button drives, and `playsinline` goes no further since Wistia plays inline anyway.
 */
