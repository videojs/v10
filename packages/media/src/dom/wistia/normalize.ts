...
/**
 * The members of Wistia's `<wistia-player>` that {@link normalizeWistiaPlayer} reads, described structurally
 * and used as its argument type: `WistiaMedia` and React both pass Wistia's own element, so a member Wistia
 * renames or drops fails to compile rather than quietly disabling a store feature at runtime.
 */
...
/**
 * Wistia's spellings for the media events it renames, against the ones a media element uses. `play`, `pause`,
 * `ended`, `seeking`, and `seeked` are absent because Wistia already spells those the way a media does.
 *
 * `durationchange` comes from three, because Wistia has no single moment where a duration appears: `duration`
 * reads `0` until `api-ready`, the media data lands on `loaded-media-data`, and `loaded-metadata` may wait on
 * the viewer interacting. `loadedmetadata` is held to once per source below, since that one is not repeatable.
 * `timeupdate` comes from two: `second-change` is a coarser subset of `time-update`, so taking both keeps the
 * clock running at whatever resolution is on offer, and a repeat costs one re-read of the playhead.
 * `progress` and `playing` ride along because Wistia announces neither.
 */
 ...
 *
 * Applied to the element rather than wrapped around it, so the player a consumer holds and the media the
 * store drives are one object — `WistiaMedia` calls this on itself, React on the element it renders.
 * Idempotent.
 *
 * `controls` is deliberately not among the members: Wistia has one already and it means the player's control
 * instances, which its internals read. Turning chrome on and off is `wistiaControlProps`.
 */
...
/**
 * An empty range that is not the shared `EMPTY_TIME_RANGES`: `isMediaBufferCapable` reads that exact object
 * as "no buffer surface at all" and skips the feature for good, where a Wistia player has one and merely has
 * nothing in it until a duration arrives.
 */
...
const WISTIA_MEDIA_DESCRIPTORS: PropertyDescriptorMap = {
  ...
  /**
   * True unless the media is playing. Wistia answers `state === 'paused'`, which is `false` in the
   * `beforeplay` state a player opens in and `false` again before its API is ready — so left alone, the first
   * click on an untouched player pauses something that was never playing.
   */
