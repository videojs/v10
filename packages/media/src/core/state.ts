...
export interface MediaMetadataState {
  ...
  poster: string;
  /**
   * The resolved placeholder URL: a low-resolution stand-in painted behind the
   * poster while it loads. Resolved independently of the poster, so a player
  ...
  /** Set the user placeholder override, or clear it with `null` or `undefined`. */
  setPlaceholder(value: string | null | undefined): void;
  /** Set the fallback used when neither the user nor media supplies a placeholder, or clear it. */
  setDefaultPlaceholder(value: string | null | undefined): void;
}
