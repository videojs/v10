 ...
 *
 * Mirrors `@videojs/mux-video`'s hls.js-backed `MuxMedia` — same class shape, same `src`/`source` relationship, same
 * derived `contentData` — over a different engine, though named for the flavor rather than sharing that class's name.
 * It carries no `engine` or `preferPlayback`: SPF publishes no engine-shaped config for a consumer to pass, so the
 * source is Mux identity and nothing else.
 *
