...
export const CDN_I18N_REGISTRY = '@videojs/cdn/i18n-registry';
 ...
 *
 * The rewritten specifier is always relative, so a mirrored copy of the package resolves entirely within its own origin
 * and works offline. Sharing one registry instance no longer depends on every bundle naming the same absolute URL —
...
export function cdnI18nExternalPlugin(options: CdnI18nExternalPluginOptions): BuildPlugin {
  ...
  function isCdnI18nEntry(importer: string): boolean {
    ...
    return normalized.includes('/cdn/src/i18n.ts') || normalized.endsWith('/cdn/i18n.js');
  }
