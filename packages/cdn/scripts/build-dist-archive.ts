/**
 * Build the distribution archives attached to each `@videojs/cdn` GitHub release.
 *
 ...
 *
 * Prerequisites: `@videojs/cdn`'s `build:cdn`. Requires `zip` and `tar` on PATH.
 */
...
async function main() {
  ...
  const name = `videojs-cdn-${pkg.version}`;
