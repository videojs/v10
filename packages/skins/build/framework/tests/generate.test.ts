import { describe, expect, it } from 'vitest';
import { canonicalRoot, loadSkinCatalog } from '../../catalog/load';
import { createFrameworkSkin } from '../generate';

describe('createFrameworkSkin', () => {
  it('bundles React into one Skin module and role-based vanilla stylesheets', async () => {
    const output = await createFrameworkSkin(await loadSkinCatalog(), {
      rootDir: canonicalRoot,
      skin: 'default-video',
      projections: [{ framework: 'react' }, { framework: 'html' }],
    });
    const source = output.sources.find((candidate) => candidate.framework === 'react')!;

    expect(source.sourceFile).toBe('skin.tsx');
    expect(source.source).not.toContain("import './styles.css'");
    expect(source.source).toContain('function PlayButton$1()');
    expect(source.source).toContain('export { DefaultVideoSkin }');
    expect(source.source).not.toContain('./components/');
    expect(output.styles.map((file) => file.fileName)).toEqual([
      'styles/styles.css',
      'styles/preflight.css',
      'styles/base.css',
      'styles/theme.css',
      'styles/buttons.css',
      'styles/controls.css',
      'styles/popups.css',
      'styles/sliders.css',
    ]);
    expect(style(output, 'styles/styles.css')).toContain("@import './buttons.css'");
    expect(style(output, 'styles/buttons.css')).toContain('.media-button {');
    expect(style(output, 'styles/buttons.css').match(/^\.media-button \{/gm)).toHaveLength(1);
    expect(style(output, 'styles/buttons.css')).not.toContain('.media-play-button {');
    expect(output.styles.map((file) => file.source).join('\n')).not.toContain('--tw-');
    expect(style(output, 'styles/preflight.css')).toContain('@scope (.media-skin)');
  });

  it('bundles HTML registrations and markup into one Skin module', async () => {
    const output = await createFrameworkSkin(await loadSkinCatalog(), {
      rootDir: canonicalRoot,
      skin: 'default-video',
      projections: [{ framework: 'html' }],
    });
    const source = output.sources[0]!;

    expect(source.sourceFile).toBe('skin.ts');
    expect(source.source).toContain("import '@videojs/html/icons/element'");
    expect(source.source).toContain('export const skin = /* html */ `<media-controls');
    expect(source.source).toContain(
      '<media-play-button class="media-button media-play-button" commandfor="play-tooltip">'
    );
    expect(source.source).toContain('<media-tooltip side="top" class="media-surface media-tooltip" id="play-tooltip">');
    expect(source.source).toContain('commandfor="seek-backward-tooltip"');
    expect(source.source).toContain('commandfor="seek-forward-tooltip"');
    expect(source.source).toContain('commandfor="volume-popover"');
    expect(source.source).toContain('commandfor="fullscreen-tooltip"');
    expect(source.source).toContain('<media-time-slider class="media-slider">');
    expect(source.source).toContain(
      '<media-volume-slider class="media-slider" thumb-alignment="edge" orientation="vertical">'
    );
    expect(source.source).toContain('<media-time class="media-time" type="remaining" toggle>');
    expect(source.source).toContain('open-on-hover');
    expect(source.source).toContain('delay="200"');
    expect(source.source).toContain('close-delay="100"');
    expect(style(output, 'styles/buttons.css')).toContain('.media-button {');
    expect(style(output, 'styles/buttons.css').match(/^\.media-button \{/gm)).toHaveLength(1);
    expect(output.styles.map((file) => file.source).join('\n')).not.toContain('--tw-');
  });
});

function style(output: Awaited<ReturnType<typeof createFrameworkSkin>>, fileName: string): string {
  const file = output.styles.find((candidate) => candidate.fileName === fileName);
  if (!file) throw new Error(`Missing generated style file \`${fileName}\`.`);
  return file.source;
}
