...
export async function createFrameworkSkin(
  ...
  const skin = manifest.items.find((item) => item.name === options.skin && item.type === 'skin');
  if (!skin) throw new Error(`Skin \`${options.skin}\` does not exist.`);

  const entryFile = resolve(options.rootDir, skin.source);
  const tailwindResource = skin.resources.styles?.find((path) => path.endsWith('/tailwind.css'));
  if (!tailwindResource) throw new Error(`Skin item \`${skin.name}\` has no \`tailwind.css\` style resource.`);

  const design = await loadDesignSystem(resolve(options.rootDir, tailwindResource));
  const program = createFrameworkStyleProgram(design);
  const iconSet = options.iconSet ?? 'default';
  const source =
    options.framework === 'html'
      ? await generateHtmlSkinSource(manifest, skin.name, entryFile, iconSet, program)
