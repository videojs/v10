...
const VIRTUAL_REFERENCE = './__videojs-skins-tailwind-reference.css';

/**
 * A loaded Tailwind v4 design system. Wraps Tailwind's
 * design-system and compiler APIs behind the operations Skin style generation
 * needs. Candidate recognition is intentionally separate from CSS
...
function resolveStylesheet(id: string, base: string): string {
  ...
  // then fall back to walking up from the Skins package itself —
  // covers temp-dir test fixtures that have no local node_modules.
  const fromBase = walkUpForPackage(id, base);
  if (fromBase) return fromBase;

  const skinsDir = dirname(fileURLToPath(import.meta.url));
  const fromSkins = walkUpForPackage(id, skinsDir);
  if (fromSkins) return fromSkins;
