...
export default styles({
  ...
  rules: {
    ...
    // `data-loading` on `Slider.Thumbnail.Root` itself; the HTML root is still a plain wrapper around
    ...
    root: {
      ...
      variants: {
        // The gradient draws from the root: the React image part is a real `<img>`, which has no `::after`.
        default: [
          'left-1/2',
