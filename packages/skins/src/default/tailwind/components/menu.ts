...
const itemBase = cn(
  'flex cursor-pointer select-none items-center rounded-full py-1.5 px-3',
...
const menuTokens = cn(
  '[--menu-transition-duration:250ms] [--menu-item-transition-duration:100ms]',
...
const menuHostShell = cn(
  ...
  'box-border rounded-[1.25rem] p-1.5 overscroll-none before:hidden'
);
...
export const menu = {
  ...
  item: cn(
    itemBase,
    'group/menu-item justify-between gap-2 tabular-nums text-inherit',
  ...
  back: cn(
    itemBase,
    'mb-0.5 w-full gap-1.5 font-medium text-current/70',
