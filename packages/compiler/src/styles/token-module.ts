...
function evaluateArrayParts(node: ts.ArrayLiteralExpression, env: Map<string, TokenValue>, fromFile: string): string[] {
  ...
  for (const el of node.elements) {
    const v = evaluate(ts.isSpreadElement(el) ? el.expression : el, env, fromFile);
    if (typeof v === 'string') {
      if (v) parts.push(v);
      continue;
    }
    throw evalError(
      ...
      ts.isSpreadElement(el)
        ? 'Array spreads in token expressions must resolve to strings or arrays of strings'
        : 'Arrays in token expressions must contain strings only'
    );
  }
  return parts;
}
