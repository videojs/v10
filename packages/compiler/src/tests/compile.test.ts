...
describe('compile (no transforms)', () => {
  it('round-trips a simple TSX module', () => {
    const source = `import { Foo } from 'bar';\nexport function App() { return <Foo/>; }\n`;
    const { code } = compile(source, { target: 'react' });
...
describe('compile (transformImports — bare-string rule)', () => {
  it('rewrites the module specifier and leaves identifiers untouched', () => {
...
describe('compile (transformImports — function rule)', () => {
  it('rewrites per-identifier source and bucket-merges by resolved target', () => {
  ...
  it('renames identifiers when the rule returns a different `name`', () => {
...
describe('replace', () => {
  it('substitutes a matched element with a new tag and adds the import', () => {
    const source = `function App(){ return <Old foo="bar"/>; }`;
    const { code } = compile(source, {
      target: 'react',
      plugins: [replace({ match: byTag('Old'), with: { source: 'pkg', name: 'New' } })],
    });
    expect(code).toContain(`<New foo="bar"`);
    expect(code).toContain(`import { New } from "pkg"`);
  });

  it('preserves children when matching an open element', () => {
...
describe('wrap', () => {
  it('wraps a matched element with a new tag and adds the import', () => {
    const source = `function App(){ return <Inner/>; }`;
    const { code } = compile(source, {
      target: 'react',
      plugins: [wrap({ match: byTag('Inner'), with: { source: 'pkg', name: 'Outer' } })],
    });
...
describe('childAsProp', () => {
  it('lifts a single JSX-element child into the named prop', () => {
    const source = `function App(){ return <T><B/></T>; }`;
    const { code } = compile(source, {
      target: 'react',
      plugins: [childAsProp({ match: byTag('T'), prop: 'render' })],
    });
    expect(collapse(code)).toContain(collapse(`<T render={<B/>}/>`));
  });

  it('skips when prop is already set', () => {
  ...
  it('skips when there are multiple JSX-element children', () => {
  ...
  it('matches an array of tags via anyTag', () => {
    const source = `function App(){ return <><T1><A/></T1><T2><B/></T2></>; }`;
    const { code } = compile(source, {
      target: 'react',
      plugins: [childAsProp({ match: anyTag(['T1', 'T2']), prop: 'render' })],
    });
...
describe('addProp', () => {
  it('emits a JSX value by default and adds the import', () => {
    const source = `function App(){ return <PlayButton/>; }`;
    const { code } = compile(source, {
      target: 'react',
      plugins: [addProp({ match: byTag('PlayButton'), prop: 'render', value: { source: './button', name: 'Button' } })],
    });
    expect(collapse(code)).toContain(collapse(`<PlayButton render={<Button/>}/>`));
    expect(code).toContain(`import { Button } from "./button"`);
  });

  it('emits a bare reference when kind is "ref"', () => {
    const source = `function App(){ return <PlayButton/>; }`;
    const { code } = compile(source, {
      target: 'react',
      plugins: [
  ...
  it('skips elements where the prop is already set', () => {
  ...
  it('overwrites the existing prop when overwrite is true', () => {
    const source = `function App(){ return <PlayButton render={<X/>}/>; }`;
    const { code } = compile(source, {
      target: 'react',
      plugins: [
...
describe('matchers', () => {
  it('byTag supports dotted tags', () => {
    const source = `function App(){ return <Popover.Root foo="bar"/>; }`;
    const { code } = compile(source, {
      target: 'react',
      plugins: [replace({ match: byTag('Popover.Root'), with: { source: 'pkg', name: 'NewRoot' } })],
    });
    expect(code).toContain(`<NewRoot`);
  });

  it('byTag honours `when` refinement', () => {
    ...
    const { code } = compile(source, {
      target: 'react',
      plugins: [replace({ match: byTag('Foo', { when: isA1 }), with: { source: 'pkg', name: 'Bar' } })],
    });
    expect(code).toContain(`<Bar a="1"`);
    expect(code).toContain(`<Foo a="2"`);
  });

  it('hasChild matches direct children only by default', () => {
    const source = `function App(){ return <><A><B/></A><A><div><B/></div></A></>; }`;
    const { code } = compile(source, {
      target: 'react',
      plugins: [replace({ match: byTag('A', { when: hasChild(byTag('B')) }), with: { source: 'p', name: 'Z' } })],
    });
  ...
  it('hasChild with deep:true matches descendants', () => {
  ...
  it('hasChild composes with byTag for nested shape checks', () => {
