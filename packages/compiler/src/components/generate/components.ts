...
export interface GenerateComponentsConfig {
...
export type GenerateComponentsOptions = GeneratedFileOptions;
export type GenerateComponentsResult = GeneratedFileResult;
...
export function generateComponents(
  config: GenerateComponentsConfig,
  options: GenerateComponentsOptions = {}
): GenerateComponentsResult {
  const { components, output, source } = config;
...
export function parseGenerateComponentsConfig(value: unknown, location: string): readonly GenerateComponentsConfig[] {
...
import { createComponent, defineComponents } from '@videojs/compiler/components';`;
...
export const components = defineComponents('${source}', COMPONENTS);

export type Components = typeof COMPONENTS;`;
}

function validateConfig(value: unknown, location: string): asserts value is GenerateComponentsConfig {
  ...
  if (!Array.isArray(value.components) || value.components.length === 0) {
    throw invalidConfig(location, '`components` must be a non-empty array');
  }

  for (const [index, component] of value.components.entries()) {
    if (typeof component === 'string') continue;
    if (!isPlainObject(component) || typeof component.files !== 'string' || typeof component.name !== 'function') {
      throw invalidConfig(location, `\`components[${index}]\` must be a glob or file set`);
    }
  }
}

function invalidConfig(location: string, message: string): Error {
  return new Error(`Invalid component generator config ${location}: ${message}.`);
}
