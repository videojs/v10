...
function emitNode(node: TargetNode, depth: number): string {
  if ('target' in node) return `defineTarget(${emitTarget(node.target, depth)})`;
...
function emitTarget(target: TargetReference, depth: number): string {
  const indent = '  '.repeat(depth);
  const propertyIndent = '  '.repeat(depth + 1);
  const importIndent = '  '.repeat(depth + 2);

  if ('tagName' in target) {
    const properties = [`${propertyIndent}tagName: ${quote(target.tagName)},`];
