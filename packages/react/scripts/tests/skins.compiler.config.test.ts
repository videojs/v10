...
const generatedRoot = resolve(import.meta.dirname, '../../src/__generated__/skins');
...
describe('reactSourceConfig', () => {
  ...
  it('extracts semantic classes and CSS for the vanilla target', async () => {
    ...
    expect(result.code).toContain('className="media-play-button"');
    expect(result.code).not.toContain('grid size-media-control');
    expect(result.assets[0]?.source).toContain('.media-play-button {');
  });
});
