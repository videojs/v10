...
import { PlayerExtensionHost } from '@videojs/core/dom';
 ...
 *
 * Accepts an optional store state seed. Returns the wrapper component, the mock store, a real extension host backing
 * `registerExtension`, and the context value for assertions.
 */
export function createPlayerWrapper(storeState: Record<string, unknown> = {}): {
  store: MockStore;
  extensions: PlayerExtensionHost;
  value: PlayerContextValue;
  Wrapper: ({ children }: { children: ReactNode }) => ReactNode;
} {
  const store = createMockStore(storeState);
  const extensions = new PlayerExtensionHost(() => {});
  const value: PlayerContextValue = {
    ...
    container: null,
    setContainer: vi.fn(),
    registerExtension: (extension) => extensions.add(extension),
  };
