...
import { diagnoseCompiledCandidate, diagnoseStyleIndex, formatStyleDiagnostic } from '../diagnostics';
...
describe('style diagnostics', () => {
  it('rejects peers, implicit ancestors, and unowned groups with actionable messages', () => {
    const diagnostics = diagnoseStyleIndex(
      index([
  ...
  it('allows named groups whose owners and consumers are local', () => {
    const diagnostics = diagnoseStyleIndex(
      index([rule('owner', ['group/menu']), rule('consumer', ['group-data-open/menu:block'])])
    );
  ...
  it('warns for :has() even when its named group owner is local', () => {
    const diagnostics = diagnoseStyleIndex(
      index([rule('owner', ['group/thumbnail']), rule('spinner', ['group-has-[img]/thumbnail:block'])])
    );
...
function rule(token: string, utilities: readonly string[]): StyleIndexRule {
  return {
