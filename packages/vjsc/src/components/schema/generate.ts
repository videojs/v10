...
export function createSchemaModule(options: CreateSchemaModuleOptions): SchemaModule {
  ...
  const code = `${[emitHeader(entries), emitDefinitions(entries), emitComponents(entries), emitSchema(source)].join(
    '\n\n'
  )}\n`;
...
function emitDefinitions(entries: readonly ResolvedComponent[]): string {
