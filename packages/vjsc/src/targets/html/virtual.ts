...
/** Source for the compiler-internal virtual HTML JSX runtime. */
...
export function Host(props) {
  const { children, ...attributes } = props ?? {};
  const values = [children].flat(Infinity).filter((value) => value != null && value !== false);
...
function renderAttributes(attributes, context) {
  ...
  for (const [name, value] of entries) {
    ...
    const normalized = attribute === 'class'
