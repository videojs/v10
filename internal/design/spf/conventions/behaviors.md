...
- Imperative wiring of multiple subscriptions that all want to be torn down together. Follow the [cleanup contract](#cleanup-contract); use an `AbortController` only when the underlying APIs share a signal-based lifetime.
...
When a behavior's setup — or an `effect()` body inside it — produces multiple cleanups, collect each as a named `const` and return a wrapper function that calls them. Do **not** reach for `AbortController` solely to coordinate in-behavior multi-cleanup; reserve it for APIs that already share a signal-based lifetime.
...
The runtime cost can be duplicated body code. Use a typed helper or behavior factory when it preserves narrow per-type contracts; see [Helpers and behavior factories](#helpers-and-behavior-factories).
...
> - **Consumers operate per-type** (e.g., `loadVideoSegments` reads `videoBufferActor`, `loadAudioSegments` reads `audioBufferActor`) → the per-type interface is the destination shape; the writer should split per type with a shared setup-shape helper. See [Per-type specialization](#per-type-specialization) and `change-spf-behavior` for the audit workflow.
>
> A behavior whose body iterates a per-type pair *and whose downstream consumers split per-type* is a split candidate, not an in-place cleanup. `setup-sourcebuffer.ts` is the worked example: explicit `MediaTrackType` axis, per-type write loop, per-type consumers (`loadVideoSegments` / `loadAudioSegments`) — per-type split with a shared `setupSourceBuffer` helper is the destination shape, with the Firefox `mozHasAudio` atomicity invariant as the cross-boundary audit's headline item.
...
Per [`packages/spf/src/AGENTS.md`](../../../../packages/spf/src/AGENTS.md):
...
Actor factories the Behaviors instantiate live alongside in `playback/actors/` (or `playback/actors/dom/`). Cross-imports between sibling Behaviors and Actors are expected; reaching across `media/`, `network/`, or `core/` follows the dependency rules in `packages/spf/src/AGENTS.md` and the boundary checker.
...
This is the inverse of the smell already documented in [`packages/spf/src/AGENTS.md`](../../../../packages/spf/src/AGENTS.md) (a primitive that reaches into `core/`). Same layering principle, opposite direction.
