...
- **`Locale`** — `(typeof LOCALES)[number] | (string & {})` for any BCP 47 tag.
...
- *`<media-text>Play</media-text>` inside each tooltip* — duplicates the control's translation key; two sources of truth for the same string.
