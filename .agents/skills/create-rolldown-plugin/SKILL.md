...
8. When a transform consumes `meta.magicString`, explicitly enable `experimental.nativeMagicString: true` in the owning Rolldown input config. In tsdown, configure it under `inputOptions.experimental`. Return the native object rather than stringifying it.
