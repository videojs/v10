...
3. Enable `build.rolldownOptions.experimental.nativeMagicString: true` in repository-owned Vite builds. This affects Vite's production Rolldown build but does not add Rolldown AST or MagicString metadata to Vite transform hooks; use the VJSC Vite adapter for transforms that need it.
