...
Structure and conventions for API reference MDX pages at `site/src/content/docs/components/`.
...
```mdx
<FrameworkCase frameworks={["react"]}>
Pass `selectPlayback` to <DocsLink slug="components/use-player">`usePlayer`</DocsLink> to subscribe.
</FrameworkCase>

<FrameworkCase frameworks={["html"]}>
Pass `selectPlayback` to <DocsLink slug="components/player-controller">`PlayerController`</DocsLink> to subscribe.
</FrameworkCase>
```
