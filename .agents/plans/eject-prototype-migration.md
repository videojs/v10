...
| `packages/compiler/{package.json,tsconfig.json,pack.config.ts,test.config.ts,vite.config.ts}` plus root workspace references | PR 1: `eject/01-compiler-foundation` | #1957 | Recreate minimal private-package wiring; remove the prototype `vjs` bin and defer style/Tailwind entry points. |
...
| `packages/skins/package.json`, `tsconfig.json`, `pack.config.ts`, `vite.config.ts` | PR 3: `eject/03-canonical-skins` | #1953, #1955 | Add only canonical authoring dependencies and validation; keep the existing `src` build and exports unchanged. |
