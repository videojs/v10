...
- **Every cross-origin subresource has to be CORS-enabled.** StackBlitz previews are cross-origin isolated (`Cross-Origin-Embedder-Policy: require-corp`), so a no-CORS load from `stream.mux.com` or `image.mux.com` is blocked outright — neither host sends `Cross-Origin-Resource-Policy`. That is why every media element here carries `crossorigin="anonymous"` (which also puts the storyboard `<track>` into CORS mode) and every poster is an `<img crossorigin="anonymous">` rather than the skin's plain `poster={url}` string. A CSS `url()` cannot be CORS-enabled at all, so the `placeholdersrc` blur-up simply does not render in a preview.
