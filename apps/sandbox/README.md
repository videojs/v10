...
- **Only published packages may be dependencies.** CSS skins come from `@videojs/html` and `@videojs/react`. Setup uses the stock Shadcn CLI to install ignored source-owned Tailwind skins from the local built registry in the monorepo or `https://shadcn.videojs.org/r` elsewhere.
