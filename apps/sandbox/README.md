...
`src/` is gitignored and so never part of the upload. That is fine: the `dev` task runs `setup.ts` first, which recreates `src/` from `templates/` on boot.
