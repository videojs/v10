...
- **Every cross-origin subresource has to be CORS-enabled.** StackBlitz previews are cross-origin isolated (`Cross-Origin-Embedder-Policy: require-corp`), so a no-CORS load from `stream.mux.com` or `image.mux.com` is blocked outright — neither host sends `Cross-Origin-Resource-Policy`. That is why every media element here carries a bare `crossorigin` (the CORS-settings attribute treats it as `anonymous`), which also puts the storyboard `<track>` into CORS mode and, through it, the thumbnail sprites. The poster works the same way wherever a template owns the image: the HTML templates slot `<img slot="poster" crossorigin>`, and the Mux React templates hand one to `renderPoster`. React has no bare-attribute form, so its templates write `crossOrigin=""`. Two things stay broken in a preview and cannot be fixed from here: a CSS `url()` can never be CORS-enabled, so the `placeholdersrc` blur-up does not render, and a skin left to source its own poster from the `poster` URL string has nowhere to put a CORS mode.
