...
- **Every cross-origin subresource has to be CORS-enabled.** StackBlitz previews are cross-origin isolated (`Cross-Origin-Embedder-Policy: require-corp`), so a no-CORS load from `stream.mux.com` or `image.mux.com` is blocked outright — neither host sends `Cross-Origin-Resource-Policy`. That is why every media element here carries `crossorigin="anonymous"`, which also puts the storyboard `<track>` into CORS mode and, through it, the thumbnail sprites. The HTML templates slot `<img slot="poster" crossorigin="anonymous">` for the same reason. Two things stay broken in a preview and cannot be fixed from here: a CSS `url()` can never be CORS-enabled, so the `placeholdersrc` blur-up does not render, and the React skins' `poster` prop takes a URL string with no way to attach a CORS mode.
