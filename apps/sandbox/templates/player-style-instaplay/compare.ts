 ...
 * axis cannot live in the shell's Compare, whose panels all differ on a shell-owned selection, and it _has_ to be two
 * frames: the two stacks register eleven of the same custom element names.
 */
...
const PANELS = [
  ...
  { id: 'media-chrome', label: 'media-chrome', hint: 'original · @player.style/instaplay 0.1.2' },
] as const;
...
root.innerHTML = `
  <header class="border-border bg-background flex min-h-10 shrink-0 flex-wrap items-center gap-x-4 gap-y-1.5 border-b px-4 py-1.5">
    <h1 class="text-sm font-medium">Instaplay — port vs original</h1>
  ...
  <div class="min-h-0 flex-1 gap-px bg-muted/40" data-panels></div>
`;
...
function applyLayout(): void {
  panelsEl.className = `min-h-0 flex-1 gap-px bg-muted/40 ${LAYOUT_CLASSES[layout]}`;
...
/** Point every frame at its URL. Toggling the mirror reloads them, since the flag is read from the frame's own URL. */
