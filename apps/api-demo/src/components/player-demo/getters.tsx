import { findCuePointTrack, readCuePoints } from './cue-points';
...
const GETTERS: { expr: string; read: (media: TracksMedia) => string }[] = [
  ...
  // Cue points are cues on the demo's metadata track — read them straight back
  // off `media.textTracks`.
  { expr: 'cuePointTrack.cues', read: (m) => JSON.stringify(readCuePoints(findCuePointTrack(m))) },
  {
