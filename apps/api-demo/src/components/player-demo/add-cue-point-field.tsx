...
/**
 * Add a cue point, which the demo writes to its metadata `<track>` element as a
 * `VTTCue`. The time field seeks the media as you change it; the entered value
 * (or the current time when blank) becomes the cue's `startTime`.
 */
  ...
  return (
    <form
      ...
      onSubmit={(event) => {
        ...
        onAdd({ time, value });
        log('action', `track.addCue(new VTTCue(${time.toFixed(2)}, …, ${JSON.stringify(value)}))`);
        setValueDraft('');
      }}
    >
      <input
